...
export const metadata: Metadata = {
  ...
  tags: [],
  httpMethod: 'post',
  httpPath: '/subscriptions/v1/check',
  operationId: 'postSubscriptionsV1Check',
};
...
export const handler = async (client: Hercules, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(await maybeFilter(jq_filter, await client.subscriptions.check(body)));
  } catch (error) {
