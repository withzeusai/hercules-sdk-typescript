...
export const metadata: Metadata = {
  ...
  tags: [],
  httpMethod: 'post',
  httpPath: '/subscriptions/v1/customers',
  operationId: 'postSubscriptionsV1Customers',
};

export const tool: Tool = {
  name: 'create_subscriptions_customers',
  description:
...
export const handler = async (client: Hercules, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(
      await maybeFilter(jq_filter, await client.subscriptions.customers.create(body)),
    );
  } catch (error) {
