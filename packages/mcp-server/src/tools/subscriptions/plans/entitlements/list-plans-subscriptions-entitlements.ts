...
export const metadata: Metadata = {
  ...
  operationId: 'getSubscriptionsV1Plans:plan_idEntitlements',
};

export const tool: Tool = {
  name: 'list_plans_subscriptions_entitlements',
  description:
...
export const handler = async (client: Hercules, args: Record<string, unknown> | undefined) => {
  ...
  const response = await client.subscriptions.plans.entitlements.list(plan_id, body).asResponse();
  try {
