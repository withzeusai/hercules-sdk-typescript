...
export const metadata: Metadata = {
  ...
  operationId: 'getSubscriptionsV1Plans:plan_id',
};

export const tool: Tool = {
  name: 'get_subscriptions_plans',
  description:
...
export const handler = async (client: Hercules, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(await maybeFilter(jq_filter, await client.subscriptions.plans.get(plan_id)));
  } catch (error) {
