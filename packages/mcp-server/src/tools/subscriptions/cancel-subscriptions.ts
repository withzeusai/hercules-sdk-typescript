...
export const metadata: Metadata = {
  ...
  tags: [],
  httpMethod: 'post',
  httpPath: '/subscriptions/v1/cancel',
  operationId: 'postSubscriptionsV1Cancel',
};
...
export const handler = async (client: Hercules, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(await maybeFilter(jq_filter, await client.subscriptions.cancel(body)));
  } catch (error) {
