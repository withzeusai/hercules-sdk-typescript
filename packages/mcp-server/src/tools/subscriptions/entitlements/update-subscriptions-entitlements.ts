...
export const metadata: Metadata = {
  ...
  operationId: 'patchSubscriptionsV1Entitlements:entitlement_id',
};

export const tool: Tool = {
  name: 'update_subscriptions_entitlements',
  description:
...
export const handler = async (client: Hercules, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(
      await maybeFilter(jq_filter, await client.subscriptions.entitlements.update(entitlement_id, body)),
    );
  } catch (error) {
