...
export const metadata: Metadata = {
  ...
  tags: [],
  httpMethod: 'post',
  httpPath: '/subscriptions/v1/entitlements',
  operationId: 'postSubscriptionsV1Entitlements',
};

export const tool: Tool = {
  name: 'create_subscriptions_entitlements',
  description:
...
export const handler = async (client: Hercules, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(
      await maybeFilter(jq_filter, await client.subscriptions.entitlements.create(body)),
    );
  } catch (error) {
