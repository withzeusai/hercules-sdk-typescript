...
export const metadata: Metadata = {
  ...
  operationId: 'getSubscriptionsV1Coupons:coupon_id',
};

export const tool: Tool = {
  name: 'get_subscriptions_coupons',
  description:
...
export const handler = async (client: Hercules, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(
      await maybeFilter(jq_filter, await client.subscriptions.coupons.get(coupon_id)),
    );
  } catch (error) {
