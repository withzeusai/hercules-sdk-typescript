...
export const metadata: Metadata = {
  ...
  operationId: 'patchSubscriptionsV1Coupons:coupon_id',
};

export const tool: Tool = {
  name: 'update_subscriptions_coupons',
  description:
...
export const handler = async (client: Hercules, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(
      await maybeFilter(jq_filter, await client.subscriptions.coupons.update(coupon_id, body)),
    );
  } catch (error) {
