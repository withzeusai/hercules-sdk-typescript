...
export { ClientOptions } from '@usehercules/sdk';
...
async function getInstructions() {
  ...
  const response = await fetch(
    readEnv('CODE_MODE_INSTRUCTIONS_URL') ?? 'https://api.stainless.com/api/ai/instructions/hercules',
    {
      method: 'GET',
  ...
  if (!response.ok) {
    ...
    instructions = `
      This is the hercules MCP server. You will use Code Mode to help the user perform
...
export async function initMcpServer(params: {
  ...
  let client = new Hercules({
    ...{ apiVersion: readEnvOrError('HERCULES_API_VERSION') as ClientOptions['apiVersion'] },
    logger,
