...
const EMBEDDED_METHODS: MethodEntry[] = [
  ...
  },
  {
    ...
    params: ['actor_user_id: string;', 'invitation_token: string;'],
    response:
      '{ convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }; invitation_id: string; membership_id: string; role_assignment_ids: string[]; tenant_id: string; }',
    markdown:
      "## accept\n\n`client.iam.invitations.accept(actor_user_id: string, invitation_token: string): { convex_source_data: object; invitation_id: string; membership_id: string; role_assignment_ids: string[]; tenant_id: string; }`\n\n**post** `/v1/iam/invitations/accept`\n\nAccepts a pending invitation as the signed-in end user and materializes its tenant-wide role assignments.\n\n### Parameters\n\n- `actor_user_id: string`\n  The signed-in end user's ID (their OIDC subject), asserted by the trusted app backend.\n\n- `invitation_token: string`\n  Secret invitation token.\n\n### Returns\n\n- `{ convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }; invitation_id: string; membership_id: string; role_assignment_ids: string[]; tenant_id: string; }`\n  Accepted invitation.\n\n  - `convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }`\n  - `invitation_id: string`\n  - `membership_id: string`\n  - `role_assignment_ids: string[]`\n  - `tenant_id: string`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst response = await client.iam.invitations.accept({ actor_user_id: 'x', invitation_token: 'x' });\n\nconsole.log(response);\n```",
    perLanguage: {
      typescript: {
  ...
  },
  {
    ...
    description: "Lists the deployment's IAM tenants, primary tenant first.",
    stainlessPath: '(resource) iam.tenants > (method) list',
    qualified: 'client.iam.tenants.list',
    params: ['limit?: number;', 'starting_after?: string;', "status?: 'active' | 'archived';"],
    response:
      "{ data: { access_mode: 'open' | 'allowlisted_only' | 'invite_only' | 'approval_required'; created_at: string; default_role_id: string; is_primary_tenant: boolean; name: string; status: 'active' | 'archived'; tenant_id: string; updated_at: string; }[]; has_more: boolean; }",
    markdown:
      "## list\n\n`client.iam.tenants.list(limit?: number, starting_after?: string, status?: 'active' | 'archived'): { data: object[]; has_more: boolean; }`\n\n**get** `/v1/iam/tenants`\n\nLists the deployment's IAM tenants, primary tenant first.\n\n### Parameters\n\n- `limit?: number`\n  Maximum number of records to return. Defaults to 50.\n\n- `starting_after?: string`\n  Cursor for forward pagination. Pass the ID of the last item from the previous page.\n\n- `status?: 'active' | 'archived'`\n  Filter by tenant status.\n\n### Returns\n\n- `{ data: { access_mode: 'open' | 'allowlisted_only' | 'invite_only' | 'approval_required'; created_at: string; default_role_id: string; is_primary_tenant: boolean; name: string; status: 'active' | 'archived'; tenant_id: string; updated_at: string; }[]; has_more: boolean; }`\n  Paginated IAM tenants.\n\n  - `data: { access_mode: 'open' | 'allowlisted_only' | 'invite_only' | 'approval_required'; created_at: string; default_role_id: string; is_primary_tenant: boolean; name: string; status: 'active' | 'archived'; tenant_id: string; updated_at: string; }[]`\n  - `has_more: boolean`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst tenants = await client.iam.tenants.list();\n\nconsole.log(tenants);\n```",
    perLanguage: {
      typescript: {
  ...
  },
  {
    ...
    response:
      '{ convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }; default_role_id: string; idempotent: boolean; membership_id: string; tenant_id: string; }',
    markdown:
      "## create\n\n`client.iam.tenants.create(actor_user_id: string, name: string, access_mode?: 'open' | 'allowlisted_only' | 'invite_only' | 'approval_required', default_role?: { id: string; } | { key: string; }, owner_user_id?: string): { convex_source_data: object; default_role_id: string; idempotent: boolean; membership_id: string; tenant_id: string; }`\n\n**post** `/v1/iam/tenants`\n\nCreates a tenant and assigns its initial owner. The signed-in user becomes the owner unless trusted server code specifies another user. The initial owner is granted the seeded owner role. The default role (for later members) defaults to the seeded member role and must not be an app-scoped role.\n\n### Parameters\n\n- `actor_user_id: string`\n  The signed-in end user's ID (their OIDC subject), asserted by the trusted app backend. Used for identity and audit only.\n\n- `name: string`\n  Human-readable tenant name.\n\n- `access_mode?: 'open' | 'allowlisted_only' | 'invite_only' | 'approval_required'`\n  Initial tenant access mode. Defaults to `open`.\n\n- `default_role?: { id: string; } | { key: string; }`\n  Identifies exactly one IAM role by ID or stable key.\n\n- `owner_user_id?: string`\n  The end user's ID (their OIDC subject) to grant the initial owner role. Required when using service authority (actor_user_id null); with user authority the signed-in user becomes the owner.\n\n### Returns\n\n- `{ convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }; default_role_id: string; idempotent: boolean; membership_id: string; tenant_id: string; }`\n  Created IAM tenant.\n\n  - `convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }`\n  - `default_role_id: string`\n  - `idempotent: boolean`\n  - `membership_id: string`\n  - `tenant_id: string`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst tenant = await client.iam.tenants.create({ actor_user_id: 'x', name: 'x' });\n\nconsole.log(tenant);\n```",
    perLanguage: {
      typescript: {
        method: 'client.iam.tenants.create',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst tenant = await client.iam.tenants.create({ actor_user_id: 'x', name: 'x' });\n\nconsole.log(tenant.default_role_id);",
      },
      http: {
        example:
          'curl https://api.hercules.app/v1/iam/tenants \\\n    -H \'Content-Type: application/json\' \\\n    -H "Authorization: Bearer $HERCULES_API_KEY" \\\n    -d \'{\n          "actor_user_id": "x",\n          "name": "x"\n        }\'',
  ...
  },
  {
    ...
    httpMethod: 'get',
    summary: 'Get tenant',
    description: "Returns one IAM tenant by ID. Pass `primary` for the deployment's primary tenant.",
    stainlessPath: '(resource) iam.tenants > (method) get',
    qualified: 'client.iam.tenants.get',
    params: ['tenant_id: string;'],
    response:
      "{ access_mode: 'open' | 'allowlisted_only' | 'invite_only' | 'approval_required'; created_at: string; default_role_id: string; is_primary_tenant: boolean; name: string; status: 'active' | 'archived'; tenant_id: string; updated_at: string; }",
    markdown:
      "## get\n\n`client.iam.tenants.get(tenant_id: string): { access_mode: 'open' | 'allowlisted_only' | 'invite_only' | 'approval_required'; created_at: string; default_role_id: string; is_primary_tenant: boolean; name: string; status: 'active' | 'archived'; tenant_id: string; updated_at: string; }`\n\n**get** `/v1/iam/tenants/{tenant_id}`\n\nReturns one IAM tenant by ID. Pass `primary` for the deployment's primary tenant.\n\n### Parameters\n\n- `tenant_id: string`\n\n### Returns\n\n- `{ access_mode: 'open' | 'allowlisted_only' | 'invite_only' | 'approval_required'; created_at: string; default_role_id: string; is_primary_tenant: boolean; name: string; status: 'active' | 'archived'; tenant_id: string; updated_at: string; }`\n  One IAM tenant.\n\n  - `access_mode: 'open' | 'allowlisted_only' | 'invite_only' | 'approval_required'`\n  - `created_at: string`\n  - `default_role_id: string`\n  - `is_primary_tenant: boolean`\n  - `name: string`\n  - `status: 'active' | 'archived'`\n  - `tenant_id: string`\n  - `updated_at: string`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst tenant = await client.iam.tenants.get('tenant_id');\n\nconsole.log(tenant);\n```",
    perLanguage: {
      typescript: {
  ...
  },
  {
    ...
    description: "Updates a tenant's name, default role, or access mode.",
    ...
    response:
      '{ convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }; tenant_id: string; }',
    markdown:
      "## update\n\n`client.iam.tenants.update(tenant_id: string, actor_user_id: string, access_mode?: 'open' | 'allowlisted_only' | 'invite_only' | 'approval_required', default_role?: { id: string; } | { key: string; }, name?: string): { convex_source_data: object; tenant_id: string; }`\n\n**patch** `/v1/iam/tenants/{tenant_id}`\n\nUpdates a tenant's name, default role, or access mode.\n\n### Parameters\n\n- `tenant_id: string`\n\n- `actor_user_id: string`\n  The signed-in end user's ID (their OIDC subject), asserted by the trusted app backend. Used for identity and audit only.\n\n- `access_mode?: 'open' | 'allowlisted_only' | 'invite_only' | 'approval_required'`\n  New tenant access mode.\n\n- `default_role?: { id: string; } | { key: string; }`\n  Identifies exactly one IAM role by ID or stable key.\n\n- `name?: string`\n  New tenant name.\n\n### Returns\n\n- `{ convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }; tenant_id: string; }`\n  Result of an IAM tenant mutation.\n\n  - `convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }`\n  - `tenant_id: string`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst tenant = await client.iam.tenants.update('tenant_id', { actor_user_id: 'x' });\n\nconsole.log(tenant);\n```",
    perLanguage: {
      typescript: {
  ...
  },
  {
    ...
    description: 'Archives a non-primary tenant and blocks its access without deleting its data.',
    stainlessPath: '(resource) iam.tenants > (method) archive',
    qualified: 'client.iam.tenants.archive',
    params: ['tenant_id: string;', 'actor_user_id: string;'],
    response:
      '{ convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }; tenant_id: string; }',
    markdown:
      "## archive\n\n`client.iam.tenants.archive(tenant_id: string, actor_user_id: string): { convex_source_data: object; tenant_id: string; }`\n\n**post** `/v1/iam/tenants/{tenant_id}/archive`\n\nArchives a non-primary tenant and blocks its access without deleting its data.\n\n### Parameters\n\n- `tenant_id: string`\n\n- `actor_user_id: string`\n  The signed-in end user's ID (their OIDC subject), asserted by the trusted app backend. Used for identity and audit only.\n\n### Returns\n\n- `{ convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }; tenant_id: string; }`\n  Result of an IAM tenant mutation.\n\n  - `convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }`\n  - `tenant_id: string`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst response = await client.iam.tenants.archive('tenant_id', { actor_user_id: 'x' });\n\nconsole.log(response);\n```",
    perLanguage: {
      typescript: {
  ...
  },
  {
    ...
    description: 'Restores an archived tenant and re-enables access through its existing data.',
    ...
    response:
      '{ convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }; tenant_id: string; }',
    markdown:
      "## unarchive\n\n`client.iam.tenants.unarchive(tenant_id: string, actor_user_id: string): { convex_source_data: object; tenant_id: string; }`\n\n**post** `/v1/iam/tenants/{tenant_id}/unarchive`\n\nRestores an archived tenant and re-enables access through its existing data.\n\n### Parameters\n\n- `tenant_id: string`\n\n- `actor_user_id: string`\n  The signed-in end user's ID (their OIDC subject), asserted by the trusted app backend. Used for identity and audit only.\n\n### Returns\n\n- `{ convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }; tenant_id: string; }`\n  Result of an IAM tenant mutation.\n\n  - `convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }`\n  - `tenant_id: string`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst response = await client.iam.tenants.unarchive('tenant_id', { actor_user_id: 'x' });\n\nconsole.log(response);\n```",
    perLanguage: {
      typescript: {
  ...
  },
  {
    ...
    params: ['tenant_id: string;', 'actor_user_id: string;'],
    response:
      "{ allowed: boolean; convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }; membership_id: string; reason: 'deny_rule' | 'not_allowlisted' | 'invite_only' | 'tenant_archived'; status: 'active' | 'pending_approval' | 'denied'; tenant_id: string; user_id: string; }",
    markdown:
      "## evaluate_access\n\n`client.iam.tenants.evaluateAccess(tenant_id: string, actor_user_id: string): { allowed: boolean; convex_source_data: object; membership_id: string; reason: 'deny_rule' | 'not_allowlisted' | 'invite_only' | 'tenant_archived'; status: 'active' | 'pending_approval' | 'denied'; tenant_id: string; user_id: string; }`\n\n**post** `/v1/iam/tenants/{tenant_id}/evaluate-access`\n\nEvaluates whether the signed-in end user may enter the tenant and applies the result: access mode `open` creates an active membership with the tenant default role, `approval_required` creates a pending membership, and denials create nothing. Call it after sign-in, before reading the user's access status. Safe to repeat; an existing membership is returned unchanged.\n\n### Parameters\n\n- `tenant_id: string`\n\n- `actor_user_id: string`\n  The signed-in end user's ID (their OIDC subject), asserted by the trusted app backend.\n\n### Returns\n\n- `{ allowed: boolean; convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }; membership_id: string; reason: 'deny_rule' | 'not_allowlisted' | 'invite_only' | 'tenant_archived'; status: 'active' | 'pending_approval' | 'denied'; tenant_id: string; user_id: string; }`\n  Tenant entry decision for one end user.\n\n  - `allowed: boolean`\n  - `convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }`\n  - `membership_id: string`\n  - `reason: 'deny_rule' | 'not_allowlisted' | 'invite_only' | 'tenant_archived'`\n  - `status: 'active' | 'pending_approval' | 'denied'`\n  - `tenant_id: string`\n  - `user_id: string`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst response = await client.iam.tenants.evaluateAccess('tenant_id', { actor_user_id: 'x' });\n\nconsole.log(response);\n```",
    perLanguage: {
      typescript: {
  ...
  },
  {
    ...
    description: 'Creates an invitation that confers tenant-wide roles when accepted.',
    stainlessPath: '(resource) iam.tenants > (method) create_invitation',
    qualified: 'client.iam.tenants.createInvitation',
    params: [
      'tenant_id: string;',
      'actor_user_id: string;',
      "constraint?: { type: 'email'; value: string; } | { type: 'domain'; value: string; };",
      'delivery?: { to_emails: string[]; from_email?: string; };',
      'expires_at?: string;',
      'max_uses?: number;',
  ...
  },
  {
    ...
    endpoint: '/v1/iam/tenants/{tenant_id}/role-assignments',
    httpMethod: 'get',
    summary: 'List tenant role assignments',
    description:
      'Lists tenant-wide role assignments in a tenant, newest first. Filter by member, group, or role.',
    stainlessPath: '(resource) iam.tenants > (method) list_role_assignments',
    qualified: 'client.iam.tenants.listRoleAssignments',
    params: [
      'tenant_id: string;',
      'group_id?: string;',
      'limit?: number;',
    ...
    response:
      "{ data: { created_at: string; expires_at: string; role_id: string; subject: { member_role_assignment_id: string; membership_id: string; type: 'member'; } | { group_id: string; group_role_assignment_id: string; type: 'group'; }; tenant_id: string; }[]; has_more: boolean; }",
    markdown:
      "## list_role_assignments\n\n`client.iam.tenants.listRoleAssignments(tenant_id: string, group_id?: string, limit?: number, membership_id?: string, role_id?: string, starting_after?: string): { data: object[]; has_more: boolean; }`\n\n**get** `/v1/iam/tenants/{tenant_id}/role-assignments`\n\nLists tenant-wide role assignments in a tenant, newest first. Filter by member, group, or role.\n\n### Parameters\n\n- `tenant_id: string`\n\n- `group_id?: string`\n  Return only this group's assignments.\n\n- `limit?: number`\n  Maximum number of records to return. Defaults to 50.\n\n- `membership_id?: string`\n  Return only this tenant membership's assignments.\n\n- `role_id?: string`\n  Return only assignments of this role.\n\n- `starting_after?: string`\n  Cursor for forward pagination. Pass the ID of the last item from the previous page.\n\n### Returns\n\n- `{ data: { created_at: string; expires_at: string; role_id: string; subject: { member_role_assignment_id: string; membership_id: string; type: 'member'; } | { group_id: string; group_role_assignment_id: string; type: 'group'; }; tenant_id: string; }[]; has_more: boolean; }`\n  Paginated tenant-wide role assignments.\n\n  - `data: { created_at: string; expires_at: string; role_id: string; subject: { member_role_assignment_id: string; membership_id: string; type: 'member'; } | { group_id: string; group_role_assignment_id: string; type: 'group'; }; tenant_id: string; }[]`\n  - `has_more: boolean`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst response = await client.iam.tenants.listRoleAssignments('tenant_id');\n\nconsole.log(response);\n```",
    perLanguage: {
      typescript: {
        method: 'client.iam.tenants.listRoleAssignments',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst response = await client.iam.tenants.listRoleAssignments('tenant_id');\n\nconsole.log(response.data);",
      },
      http: {
        example:
          'curl https://api.hercules.app/v1/iam/tenants/$TENANT_ID/role-assignments \\\n    -H "Authorization: Bearer $HERCULES_API_KEY"',
      },
    },
  },
  {
    name: 'list_resource_role_assignments',
    endpoint: '/v1/iam/tenants/{tenant_id}/resource-role-assignments',
    httpMethod: 'get',
    summary: 'List tenant resource role assignments',
    description:
    ...
    stainlessPath: '(resource) iam.tenants > (method) list_resource_role_assignments',
    qualified: 'client.iam.tenants.listResourceRoleAssignments',
    params: [
      'tenant_id: string;',
      'external_id?: string;',
      'group_id?: string;',
    ...
    response:
      "{ data: { created_at: string; expires_at: string; external_id: string; resource_type_id: string; role_id: string; subject: { member_resource_role_assignment_id: string; membership_id: string; type: 'member'; } | { group_id: string; group_resource_role_assignment_id: string; type: 'group'; }; tenant_id: string; }[]; has_more: boolean; }",
    markdown:
      "## list_resource_role_assignments\n\n`client.iam.tenants.listResourceRoleAssignments(tenant_id: string, external_id?: string, group_id?: string, limit?: number, membership_id?: string, resource_type_id?: string, resource_type_key?: string, role_id?: string, starting_after?: string): { data: object[]; has_more: boolean; }`\n\n**get** `/v1/iam/tenants/{tenant_id}/resource-role-assignments`\n\nLists resource role assignments in a tenant, newest first. Filter by resource type and external ID to find who has a role on one exact resource, or by member, group, or role.\n\n### Parameters\n\n- `tenant_id: string`\n\n- `external_id?: string`\n  Filter to one exact resource's external ID. Pair with a resource type to find who has a role on that resource.\n\n- `group_id?: string`\n  Return only this group's assignments.\n\n- `limit?: number`\n  Maximum number of records to return. Defaults to 50.\n\n- `membership_id?: string`\n  Return only this tenant membership's assignments.\n\n- `resource_type_id?: string`\n  Filter to one resource type by ID. Not combinable with resource_type_key.\n\n- `resource_type_key?: string`\n  Filter to one resource type by its catalog key (e.g. `app.project`). Not combinable with resource_type_id.\n\n- `role_id?: string`\n  Return only assignments of this role.\n\n- `starting_after?: string`\n  Cursor for forward pagination. Pass the ID of the last item from the previous page.\n\n### Returns\n\n- `{ data: { created_at: string; expires_at: string; external_id: string; resource_type_id: string; role_id: string; subject: { member_resource_role_assignment_id: string; membership_id: string; type: 'member'; } | { group_id: string; group_resource_role_assignment_id: string; type: 'group'; }; tenant_id: string; }[]; has_more: boolean; }`\n  Paginated resource role assignments.\n\n  - `data: { created_at: string; expires_at: string; external_id: string; resource_type_id: string; role_id: string; subject: { member_resource_role_assignment_id: string; membership_id: string; type: 'member'; } | { group_id: string; group_resource_role_assignment_id: string; type: 'group'; }; tenant_id: string; }[]`\n  - `has_more: boolean`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst response = await client.iam.tenants.listResourceRoleAssignments('tenant_id');\n\nconsole.log(response);\n```",
    perLanguage: {
      typescript: {
        method: 'client.iam.tenants.listResourceRoleAssignments',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst response = await client.iam.tenants.listResourceRoleAssignments('tenant_id');\n\nconsole.log(response.data);",
      },
      http: {
        example:
          'curl https://api.hercules.app/v1/iam/tenants/$TENANT_ID/resource-role-assignments \\\n    -H "Authorization: Bearer $HERCULES_API_KEY"',
      },
    },
  },
  {
    name: 'create',
    endpoint: '/v1/iam/tenants/{tenant_id}/members',
    httpMethod: 'post',
    summary: 'Add tenant member',
    description: 'Adds an end user to the tenant, optionally with a role.',
    stainlessPath: '(resource) iam.tenants.members > (method) create',
    qualified: 'client.iam.tenants.members.create',
    params: [
      'tenant_id: string;',
      'actor_user_id: string;',
      'user_id: string;',
    ...
    response:
      '{ convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }; membership_id: string; role_assignment_id: string; }',
    markdown:
      "## create\n\n`client.iam.tenants.members.create(tenant_id: string, actor_user_id: string, user_id: string, role?: { id: string; } | { key: string; }, status?: 'active' | 'blocked' | 'suspended'): { convex_source_data: object; membership_id: string; role_assignment_id: string; }`\n\n**post** `/v1/iam/tenants/{tenant_id}/members`\n\nAdds an end user to the tenant, optionally with a role.\n\n### Parameters\n\n- `tenant_id: string`\n\n- `actor_user_id: string`\n  The signed-in end user's ID (their OIDC subject), asserted by the trusted app backend. Used for identity and audit only.\n\n- `user_id: string`\n  The end user's ID (their OIDC subject) to add to the tenant.\n\n- `role?: { id: string; } | { key: string; }`\n  Identifies exactly one IAM role by ID or stable key.\n\n- `status?: 'active' | 'blocked' | 'suspended'`\n  Initial membership status. Defaults to `active`.\n\n### Returns\n\n- `{ convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }; membership_id: string; role_assignment_id: string; }`\n  Added tenant member.\n\n  - `convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }`\n  - `membership_id: string`\n  - `role_assignment_id: string`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst member = await client.iam.tenants.members.create('tenant_id', { actor_user_id: 'x', user_id: 'x' });\n\nconsole.log(member);\n```",
    perLanguage: {
      typescript: {
        method: 'client.iam.tenants.members.create',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst member = await client.iam.tenants.members.create('tenant_id', {\n  actor_user_id: 'x',\n  user_id: 'x',\n});\n\nconsole.log(member.membership_id);",
      },
      http: {
        example:
          'curl https://api.hercules.app/v1/iam/tenants/$TENANT_ID/members \\\n    -H \'Content-Type: application/json\' \\\n    -H "Authorization: Bearer $HERCULES_API_KEY" \\\n    -d \'{\n          "actor_user_id": "x",\n          "user_id": "x"\n        }\'',
      },
    },
  },
  {
    ...
    stainlessPath: '(resource) iam.tenants.members > (method) list',
    qualified: 'client.iam.tenants.members.list',
    params: [
      'tenant_id: string;',
      'limit?: number;',
      'role_id?: string;',
    ...
    response:
      "{ data: { created_at: string; membership_id: string; status: 'active' | 'blocked' | 'suspended' | 'pending_approval' | 'removed'; tenant_id: string; user_email: string; user_id: string; user_image: string; user_name: string; }[]; has_more: boolean; }",
    markdown:
      "## list\n\n`client.iam.tenants.members.list(tenant_id: string, limit?: number, role_id?: string, starting_after?: string, status?: 'active' | 'blocked' | 'suspended' | 'pending_approval' | 'removed', user_id?: string): { data: object[]; has_more: boolean; }`\n\n**get** `/v1/iam/tenants/{tenant_id}/members`\n\nLists a tenant's members, newest first. Filter by status, by a role the member directly holds, or by the member's Hercules Auth user id.\n\n### Parameters\n\n- `tenant_id: string`\n\n- `limit?: number`\n  Maximum number of records to return. Defaults to 50.\n\n- `role_id?: string`\n  Return only members that directly hold this tenant-wide role.\n\n- `starting_after?: string`\n  Cursor for forward pagination. Pass the ID of the last item from the previous page.\n\n- `status?: 'active' | 'blocked' | 'suspended' | 'pending_approval' | 'removed'`\n  Filter by membership status.\n\n- `user_id?: string`\n  Return the member for this Hercules Auth user id (the end user's OIDC subject); an exact match.\n\n### Returns\n\n- `{ data: { created_at: string; membership_id: string; status: 'active' | 'blocked' | 'suspended' | 'pending_approval' | 'removed'; tenant_id: string; user_email: string; user_id: string; user_image: string; user_name: string; }[]; has_more: boolean; }`\n  Paginated tenant members.\n\n  - `data: { created_at: string; membership_id: string; status: 'active' | 'blocked' | 'suspended' | 'pending_approval' | 'removed'; tenant_id: string; user_email: string; user_id: string; user_image: string; user_name: string; }[]`\n  - `has_more: boolean`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst members = await client.iam.tenants.members.list('tenant_id');\n\nconsole.log(members);\n```",
    perLanguage: {
      typescript: {
        method: 'client.iam.tenants.members.list',
        example:
  ...
  {
    name: 'get',
    endpoint: '/v1/iam/tenants/{tenant_id}/members/{membership_id}',
    httpMethod: 'get',
    summary: 'Get tenant member',
    description: 'Returns one tenant member by their membership ID.',
    stainlessPath: '(resource) iam.tenants.members > (method) get',
    qualified: 'client.iam.tenants.members.get',
    params: ['tenant_id: string;', 'membership_id: string;'],
    response:
      "{ created_at: string; membership_id: string; status: 'active' | 'blocked' | 'suspended' | 'pending_approval' | 'removed'; tenant_id: string; user_email: string; user_id: string; user_image: string; user_name: string; }",
    markdown:
      "## get\n\n`client.iam.tenants.members.get(tenant_id: string, membership_id: string): { created_at: string; membership_id: string; status: 'active' | 'blocked' | 'suspended' | 'pending_approval' | 'removed'; tenant_id: string; user_email: string; user_id: string; user_image: string; user_name: string; }`\n\n**get** `/v1/iam/tenants/{tenant_id}/members/{membership_id}`\n\nReturns one tenant member by their membership ID.\n\n### Parameters\n\n- `tenant_id: string`\n\n- `membership_id: string`\n\n### Returns\n\n- `{ created_at: string; membership_id: string; status: 'active' | 'blocked' | 'suspended' | 'pending_approval' | 'removed'; tenant_id: string; user_email: string; user_id: string; user_image: string; user_name: string; }`\n  One tenant member.\n\n  - `created_at: string`\n  - `membership_id: string`\n  - `status: 'active' | 'blocked' | 'suspended' | 'pending_approval' | 'removed'`\n  - `tenant_id: string`\n  - `user_email: string`\n  - `user_id: string`\n  - `user_image: string`\n  - `user_name: string`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst member = await client.iam.tenants.members.get('membership_id', { tenant_id: 'tenant_id' });\n\nconsole.log(member);\n```",
    perLanguage: {
      typescript: {
        method: 'client.iam.tenants.members.get',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst member = await client.iam.tenants.members.get('membership_id', { tenant_id: 'tenant_id' });\n\nconsole.log(member.membership_id);",
      },
      http: {
        example:
          'curl https://api.hercules.app/v1/iam/tenants/$TENANT_ID/members/$MEMBERSHIP_ID \\\n    -H "Authorization: Bearer $HERCULES_API_KEY"',
      },
    },
  },
  {
    ...
    endpoint: '/v1/iam/tenants/{tenant_id}/members/{membership_id}',
    httpMethod: 'patch',
    summary: 'Update tenant member status',
    description: 'Approves, suspends, blocks, reactivates, or removes a tenant member.',
    stainlessPath: '(resource) iam.tenants.members > (method) update_status',
    qualified: 'client.iam.tenants.members.updateStatus',
    params: [
      'tenant_id: string;',
      'membership_id: string;',
      'actor_user_id: string;',
    ...
    response:
      '{ convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }; membership_id: string; }',
    markdown:
      "## update_status\n\n`client.iam.tenants.members.updateStatus(tenant_id: string, membership_id: string, actor_user_id: string, status: 'active' | 'blocked' | 'suspended' | 'removed'): { convex_source_data: object; membership_id: string; }`\n\n**patch** `/v1/iam/tenants/{tenant_id}/members/{membership_id}`\n\nApproves, suspends, blocks, reactivates, or removes a tenant member.\n\n### Parameters\n\n- `tenant_id: string`\n\n- `membership_id: string`\n\n- `actor_user_id: string`\n  The signed-in end user's ID (their OIDC subject), asserted by the trusted app backend. Used for identity and audit only.\n\n- `status: 'active' | 'blocked' | 'suspended' | 'removed'`\n  New membership status.\n\n### Returns\n\n- `{ convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }; membership_id: string; }`\n  Result of changing a tenant member.\n\n  - `convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }`\n  - `membership_id: string`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst response = await client.iam.tenants.members.updateStatus('membership_id', {\n  tenant_id: 'tenant_id',\n  actor_user_id: 'x',\n  status: 'active',\n});\n\nconsole.log(response);\n```",
    perLanguage: {
      typescript: {
        method: 'client.iam.tenants.members.updateStatus',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst response = await client.iam.tenants.members.updateStatus('membership_id', {\n  tenant_id: 'tenant_id',\n  actor_user_id: 'x',\n  status: 'active',\n});\n\nconsole.log(response.membership_id);",
      },
      http: {
  ...
  },
  {
    ...
    description: 'Removes a member from the tenant. The component treats a removed member as denied.',
    stainlessPath: '(resource) iam.tenants.members > (method) remove',
    qualified: 'client.iam.tenants.members.remove',
    params: ['tenant_id: string;', 'membership_id: string;', 'actor_user_id?: string;'],
    response:
      '{ convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }; membership_id: string; }',
    markdown:
      "## remove\n\n`client.iam.tenants.members.remove(tenant_id: string, membership_id: string, actor_user_id?: string): { convex_source_data: object; membership_id: string; }`\n\n**delete** `/v1/iam/tenants/{tenant_id}/members/{membership_id}`\n\nRemoves a member from the tenant. The component treats a removed member as denied.\n\n### Parameters\n\n- `tenant_id: string`\n\n- `membership_id: string`\n\n- `actor_user_id?: string`\n  The signed-in end user's ID to attribute the operation to that user, or omitted for service authority.\n\n### Returns\n\n- `{ convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }; membership_id: string; }`\n  Result of changing a tenant member.\n\n  - `convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }`\n  - `membership_id: string`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst member = await client.iam.tenants.members.remove('membership_id', { tenant_id: 'tenant_id' });\n\nconsole.log(member);\n```",
    perLanguage: {
      typescript: {
        method: 'client.iam.tenants.members.remove',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst member = await client.iam.tenants.members.remove('membership_id', { tenant_id: 'tenant_id' });\n\nconsole.log(member.membership_id);",
      },
      http: {
  ...
  {
    name: 'assign_role',
    endpoint: '/v1/iam/tenants/{tenant_id}/members/{membership_id}/role-assignments',
    httpMethod: 'post',
    summary: 'Assign tenant role to member',
    description: 'Assigns a tenant-wide role to a member.',
    stainlessPath: '(resource) iam.tenants.members > (method) assign_role',
    qualified: 'client.iam.tenants.members.assignRole',
    params: [
      'tenant_id: string;',
      'membership_id: string;',
      'actor_user_id: string;',
    ...
    response:
      '{ convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }; member_role_assignment_id: string; }',
    markdown:
      "## assign_role\n\n`client.iam.tenants.members.assignRole(tenant_id: string, membership_id: string, actor_user_id: string, role: { id: string; } | { key: string; }, expires_at?: string): { convex_source_data: object; member_role_assignment_id: string; }`\n\n**post** `/v1/iam/tenants/{tenant_id}/members/{membership_id}/role-assignments`\n\nAssigns a tenant-wide role to a member.\n\n### Parameters\n\n- `tenant_id: string`\n\n- `membership_id: string`\n\n- `actor_user_id: string`\n  The signed-in end user's ID (their OIDC subject), asserted by the trusted app backend. Used for identity and audit only.\n\n- `role: { id: string; } | { key: string; }`\n  Identifies exactly one IAM role by ID or stable key.\n\n- `expires_at?: string`\n  Assignment expiry. Omit or pass null for a permanent assignment.\n\n### Returns\n\n- `{ convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }; member_role_assignment_id: string; }`\n  Result of changing a member's tenant-wide role assignment.\n\n  - `convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }`\n  - `member_role_assignment_id: string`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst response = await client.iam.tenants.members.assignRole('membership_id', {\n  tenant_id: 'tenant_id',\n  actor_user_id: 'x',\n  role: { id: 'x' },\n});\n\nconsole.log(response);\n```",
    perLanguage: {
      typescript: {
        method: 'client.iam.tenants.members.assignRole',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst response = await client.iam.tenants.members.assignRole('membership_id', {\n  tenant_id: 'tenant_id',\n  actor_user_id: 'x',\n  role: { id: 'x' },\n});\n\nconsole.log(response.member_role_assignment_id);",
      },
      http: {
        example:
          'curl https://api.hercules.app/v1/iam/tenants/$TENANT_ID/members/$MEMBERSHIP_ID/role-assignments \\\n    -H \'Content-Type: application/json\' \\\n    -H "Authorization: Bearer $HERCULES_API_KEY" \\\n    -d \'{\n          "actor_user_id": "x",\n          "role": {\n            "id": "x"\n          }\n        }\'',
      },
    },
  },
  {
    ...
    endpoint: '/v1/iam/tenants/{tenant_id}/members/{membership_id}/role-assignments',
    httpMethod: 'put',
    summary: 'Replace member tenant roles',
    description:
    ...
    stainlessPath: '(resource) iam.tenants.members > (method) replace_roles',
    qualified: 'client.iam.tenants.members.replaceRoles',
    params: [
      'tenant_id: string;',
      'membership_id: string;',
      'actor_user_id: string;',
    ...
    response:
      '{ assignments: { assignment_id: string; role_id: string; }[]; convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }; membership_id: string; }',
    markdown:
      "## replace_roles\n\n`client.iam.tenants.members.replaceRoles(tenant_id: string, membership_id: string, actor_user_id: string, roles: { id: string; } | { key: string; }[]): { assignments: object[]; convex_source_data: object; membership_id: string; }`\n\n**put** `/v1/iam/tenants/{tenant_id}/members/{membership_id}/role-assignments`\n\nReconciles the member's direct tenant-wide role assignments to exactly the supplied set in one transaction: missing roles are assigned, surplus assignments removed, matching ones kept. Roles conferred via groups are untouched. Idempotent; prefer this over unassign-then-assign loops.\n\n### Parameters\n\n- `tenant_id: string`\n\n- `membership_id: string`\n\n- `actor_user_id: string`\n  The signed-in end user's ID (their OIDC subject), asserted by the trusted app backend. Used for identity and audit only.\n\n- `roles: { id: string; } | { key: string; }[]`\n  The complete desired set of the member's direct tenant-wide roles. Missing roles are assigned, surplus assignments are removed, and roles conferred via groups are untouched. An empty array removes every direct assignment.\n\n### Returns\n\n- `{ assignments: { assignment_id: string; role_id: string; }[]; convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }; membership_id: string; }`\n  Result of replacing a member's tenant-wide role assignments.\n\n  - `assignments: { assignment_id: string; role_id: string; }[]`\n  - `convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }`\n  - `membership_id: string`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst response = await client.iam.tenants.members.replaceRoles('membership_id', {\n  tenant_id: 'tenant_id',\n  actor_user_id: 'x',\n  roles: [{ id: 'x' }],\n});\n\nconsole.log(response);\n```",
    perLanguage: {
      typescript: {
        method: 'client.iam.tenants.members.replaceRoles',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst response = await client.iam.tenants.members.replaceRoles('membership_id', {\n  tenant_id: 'tenant_id',\n  actor_user_id: 'x',\n  roles: [{ id: 'x' }],\n});\n\nconsole.log(response.membership_id);",
      },
      http: {
  ...
  {
    name: 'unassign_role',
    endpoint: '/v1/iam/tenants/{tenant_id}/members/{membership_id}/role-assignments/{assignment_id}',
    httpMethod: 'delete',
    summary: 'Unassign tenant role from member',
    description: 'Removes a tenant-wide role assignment.',
    stainlessPath: '(resource) iam.tenants.members > (method) unassign_role',
    qualified: 'client.iam.tenants.members.unassignRole',
    params: [
      'tenant_id: string;',
      'membership_id: string;',
    ...
    response:
      '{ convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }; member_role_assignment_id: string; }',
    markdown:
      "## unassign_role\n\n`client.iam.tenants.members.unassignRole(tenant_id: string, membership_id: string, assignment_id: string, actor_user_id?: string): { convex_source_data: object; member_role_assignment_id: string; }`\n\n**delete** `/v1/iam/tenants/{tenant_id}/members/{membership_id}/role-assignments/{assignment_id}`\n\nRemoves a tenant-wide role assignment.\n\n### Parameters\n\n- `tenant_id: string`\n\n- `membership_id: string`\n\n- `assignment_id: string`\n\n- `actor_user_id?: string`\n  The signed-in end user's ID to attribute the operation to that user, or omitted for service authority.\n\n### Returns\n\n- `{ convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }; member_role_assignment_id: string; }`\n  Result of changing a member's tenant-wide role assignment.\n\n  - `convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }`\n  - `member_role_assignment_id: string`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst response = await client.iam.tenants.members.unassignRole('assignment_id', { tenant_id: 'tenant_id', membership_id: 'membership_id' });\n\nconsole.log(response);\n```",
    perLanguage: {
      typescript: {
        method: 'client.iam.tenants.members.unassignRole',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst response = await client.iam.tenants.members.unassignRole('assignment_id', {\n  tenant_id: 'tenant_id',\n  membership_id: 'membership_id',\n});\n\nconsole.log(response.member_role_assignment_id);",
      },
      http: {
        example:
          'curl https://api.hercules.app/v1/iam/tenants/$TENANT_ID/members/$MEMBERSHIP_ID/role-assignments/$ASSIGNMENT_ID \\\n    -X DELETE \\\n    -H "Authorization: Bearer $HERCULES_API_KEY"',
      },
    },
  },
  {
    ...
    endpoint: '/v1/iam/tenants/{tenant_id}/members/{membership_id}/role-assignments',
    httpMethod: 'get',
    summary: "List a member's tenant role assignments",
    description: 'Lists the tenant-wide role assignments held by one member, newest first.',
    stainlessPath: '(resource) iam.tenants.members > (method) list_role_assignments',
    qualified: 'client.iam.tenants.members.listRoleAssignments',
    params: ['tenant_id: string;', 'membership_id: string;', 'limit?: number;', 'starting_after?: string;'],
    response:
      '{ data: { created_at: string; expires_at: string; member_role_assignment_id: string; role_id: string; }[]; has_more: boolean; }',
    markdown:
      "## list_role_assignments\n\n`client.iam.tenants.members.listRoleAssignments(tenant_id: string, membership_id: string, limit?: number, starting_after?: string): { data: object[]; has_more: boolean; }`\n\n**get** `/v1/iam/tenants/{tenant_id}/members/{membership_id}/role-assignments`\n\nLists the tenant-wide role assignments held by one member, newest first.\n\n### Parameters\n\n- `tenant_id: string`\n\n- `membership_id: string`\n\n- `limit?: number`\n  Maximum number of records to return. Defaults to 50.\n\n- `starting_after?: string`\n  Cursor for forward pagination. Pass the ID of the last item from the previous page.\n\n### Returns\n\n- `{ data: { created_at: string; expires_at: string; member_role_assignment_id: string; role_id: string; }[]; has_more: boolean; }`\n  Paginated tenant-wide role assignments held by one member.\n\n  - `data: { created_at: string; expires_at: string; member_role_assignment_id: string; role_id: string; }[]`\n  - `has_more: boolean`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst response = await client.iam.tenants.members.listRoleAssignments('membership_id', { tenant_id: 'tenant_id' });\n\nconsole.log(response);\n```",
    perLanguage: {
      typescript: {
        method: 'client.iam.tenants.members.listRoleAssignments',
        example:
  ...
  {
    name: 'assign_resource_role',
    endpoint: '/v1/iam/tenants/{tenant_id}/members/{membership_id}/resource-role-assignments',
    httpMethod: 'post',
    summary: 'Assign resource role to member',
    description: 'Assigns a role to a member on one exact resource.',
    stainlessPath: '(resource) iam.tenants.members > (method) assign_resource_role',
    qualified: 'client.iam.tenants.members.assignResourceRole',
    params: [
      'tenant_id: string;',
      'membership_id: string;',
      'actor_user_id: string;',
      'external_id: string;',
      'resource_type: { id: string; } | { key: string; };',
      'role: { id: string; } | { key: string; };',
      'expires_at?: string;',
    ],
    response:
      '{ convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }; member_resource_role_assignment_id: string; }',
    markdown:
      "## assign_resource_role\n\n`client.iam.tenants.members.assignResourceRole(tenant_id: string, membership_id: string, actor_user_id: string, external_id: string, resource_type: { id: string; } | { key: string; }, role: { id: string; } | { key: string; }, expires_at?: string): { convex_source_data: object; member_resource_role_assignment_id: string; }`\n\n**post** `/v1/iam/tenants/{tenant_id}/members/{membership_id}/resource-role-assignments`\n\nAssigns a role to a member on one exact resource.\n\n### Parameters\n\n- `tenant_id: string`\n\n- `membership_id: string`\n\n- `actor_user_id: string`\n  The signed-in end user's ID (their OIDC subject), asserted by the trusted app backend. Used for identity and audit only.\n\n- `external_id: string`\n  The app-defined external ID of the exact resource.\n\n- `resource_type: { id: string; } | { key: string; }`\n  Identifies exactly one IAM resource type by ID or stable key.\n\n- `role: { id: string; } | { key: string; }`\n  Identifies exactly one IAM role by ID or stable key.\n\n- `expires_at?: string`\n  Assignment expiry. Omit or pass null for a permanent assignment.\n\n### Returns\n\n- `{ convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }; member_resource_role_assignment_id: string; }`\n  Result of changing a member resource role assignment.\n\n  - `convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }`\n  - `member_resource_role_assignment_id: string`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst response = await client.iam.tenants.members.assignResourceRole('membership_id', {\n  tenant_id: 'tenant_id',\n  actor_user_id: 'x',\n  external_id: 'x',\n  resource_type: { id: 'x' },\n  role: { id: 'x' },\n});\n\nconsole.log(response);\n```",
    perLanguage: {
      typescript: {
        method: 'client.iam.tenants.members.assignResourceRole',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst response = await client.iam.tenants.members.assignResourceRole('membership_id', {\n  tenant_id: 'tenant_id',\n  actor_user_id: 'x',\n  external_id: 'x',\n  resource_type: { id: 'x' },\n  role: { id: 'x' },\n});\n\nconsole.log(response.member_resource_role_assignment_id);",
      },
      http: {
        example:
          'curl https://api.hercules.app/v1/iam/tenants/$TENANT_ID/members/$MEMBERSHIP_ID/resource-role-assignments \\\n    -H \'Content-Type: application/json\' \\\n    -H "Authorization: Bearer $HERCULES_API_KEY" \\\n    -d \'{\n          "actor_user_id": "x",\n          "external_id": "x",\n          "resource_type": {\n            "id": "x"\n          },\n          "role": {\n            "id": "x"\n          }\n        }\'',
      },
    },
  },
  {
    name: 'replace_resource_roles',
    endpoint: '/v1/iam/tenants/{tenant_id}/members/{membership_id}/resource-role-assignments',
    httpMethod: 'put',
    summary: 'Replace member resource roles',
    description:
    ...
    stainlessPath: '(resource) iam.tenants.members > (method) replace_resource_roles',
    qualified: 'client.iam.tenants.members.replaceResourceRoles',
    params: [
      'tenant_id: string;',
      'membership_id: string;',
      'actor_user_id: string;',
    ...
    response:
      '{ assignments: { assignment_id: string; role_id: string; }[]; convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }; external_id: string; membership_id: string; resource_type_id: string; }',
    markdown:
      "## replace_resource_roles\n\n`client.iam.tenants.members.replaceResourceRoles(tenant_id: string, membership_id: string, actor_user_id: string, external_id: string, resource_type: { id: string; } | { key: string; }, roles: { id: string; } | { key: string; }[]): { assignments: object[]; convex_source_data: object; external_id: string; membership_id: string; resource_type_id: string; }`\n\n**put** `/v1/iam/tenants/{tenant_id}/members/{membership_id}/resource-role-assignments`\n\nReconciles the member's direct role assignments on one exact resource to exactly the supplied set in one transaction: missing roles are assigned, surplus assignments removed, matching ones kept. Grants on other resources and via groups are untouched. Idempotent; prefer this over unassign-then-assign loops.\n\n### Parameters\n\n- `tenant_id: string`\n\n- `membership_id: string`\n\n- `actor_user_id: string`\n  The signed-in end user's ID (their OIDC subject), asserted by the trusted app backend. Used for identity and audit only.\n\n- `external_id: string`\n  The app-defined external ID of the exact resource.\n\n- `resource_type: { id: string; } | { key: string; }`\n  Identifies exactly one IAM resource type by ID or stable key.\n\n- `roles: { id: string; } | { key: string; }[]`\n  The complete desired set of the member's direct roles on this exact resource. Missing roles are assigned, surplus assignments are removed; assignments on other resources and via groups are untouched. An empty array removes every direct assignment on the resource.\n\n### Returns\n\n- `{ assignments: { assignment_id: string; role_id: string; }[]; convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }; external_id: string; membership_id: string; resource_type_id: string; }`\n  Result of replacing a member's role assignments on one resource.\n\n  - `assignments: { assignment_id: string; role_id: string; }[]`\n  - `convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }`\n  - `external_id: string`\n  - `membership_id: string`\n  - `resource_type_id: string`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst response = await client.iam.tenants.members.replaceResourceRoles('membership_id', {\n  tenant_id: 'tenant_id',\n  actor_user_id: 'x',\n  external_id: 'x',\n  resource_type: { id: 'x' },\n  roles: [{ id: 'x' }],\n});\n\nconsole.log(response);\n```",
    perLanguage: {
      typescript: {
        method: 'client.iam.tenants.members.replaceResourceRoles',
        example:
  ...
  {
    name: 'unassign_resource_role',
    endpoint: '/v1/iam/tenants/{tenant_id}/members/{membership_id}/resource-role-assignments/{assignment_id}',
    httpMethod: 'delete',
    summary: 'Unassign resource role from member',
    description: 'Removes a member resource role assignment.',
    stainlessPath: '(resource) iam.tenants.members > (method) unassign_resource_role',
    qualified: 'client.iam.tenants.members.unassignResourceRole',
    params: [
      'tenant_id: string;',
      'membership_id: string;',
      'assignment_id: string;',
    ...
    response:
      '{ convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }; member_resource_role_assignment_id: string; }',
    markdown:
      "## unassign_resource_role\n\n`client.iam.tenants.members.unassignResourceRole(tenant_id: string, membership_id: string, assignment_id: string, actor_user_id?: string): { convex_source_data: object; member_resource_role_assignment_id: string; }`\n\n**delete** `/v1/iam/tenants/{tenant_id}/members/{membership_id}/resource-role-assignments/{assignment_id}`\n\nRemoves a member resource role assignment.\n\n### Parameters\n\n- `tenant_id: string`\n\n- `membership_id: string`\n\n- `assignment_id: string`\n\n- `actor_user_id?: string`\n  The signed-in end user's ID to attribute the operation to that user, or omitted for service authority.\n\n### Returns\n\n- `{ convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }; member_resource_role_assignment_id: string; }`\n  Result of changing a member resource role assignment.\n\n  - `convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }`\n  - `member_resource_role_assignment_id: string`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst response = await client.iam.tenants.members.unassignResourceRole('assignment_id', { tenant_id: 'tenant_id', membership_id: 'membership_id' });\n\nconsole.log(response);\n```",
    perLanguage: {
      typescript: {
        method: 'client.iam.tenants.members.unassignResourceRole',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst response = await client.iam.tenants.members.unassignResourceRole('assignment_id', {\n  tenant_id: 'tenant_id',\n  membership_id: 'membership_id',\n});\n\nconsole.log(response.member_resource_role_assignment_id);",
      },
      http: {
        example:
          'curl https://api.hercules.app/v1/iam/tenants/$TENANT_ID/members/$MEMBERSHIP_ID/resource-role-assignments/$ASSIGNMENT_ID \\\n    -X DELETE \\\n    -H "Authorization: Bearer $HERCULES_API_KEY"',
      },
    },
  },
  {
    ...
    endpoint: '/v1/iam/tenants/{tenant_id}/members/{membership_id}/resource-role-assignments',
    httpMethod: 'get',
    summary: "List a member's resource role assignments",
    description: 'Lists the resource role assignments held by one member, newest first.',
    stainlessPath: '(resource) iam.tenants.members > (method) list_resource_role_assignments',
    qualified: 'client.iam.tenants.members.listResourceRoleAssignments',
    params: ['tenant_id: string;', 'membership_id: string;', 'limit?: number;', 'starting_after?: string;'],
    response:
      '{ data: { created_at: string; expires_at: string; external_id: string; member_resource_role_assignment_id: string; resource_type_id: string; role_id: string; }[]; has_more: boolean; }',
    markdown:
      "## list_resource_role_assignments\n\n`client.iam.tenants.members.listResourceRoleAssignments(tenant_id: string, membership_id: string, limit?: number, starting_after?: string): { data: object[]; has_more: boolean; }`\n\n**get** `/v1/iam/tenants/{tenant_id}/members/{membership_id}/resource-role-assignments`\n\nLists the resource role assignments held by one member, newest first.\n\n### Parameters\n\n- `tenant_id: string`\n\n- `membership_id: string`\n\n- `limit?: number`\n  Maximum number of records to return. Defaults to 50.\n\n- `starting_after?: string`\n  Cursor for forward pagination. Pass the ID of the last item from the previous page.\n\n### Returns\n\n- `{ data: { created_at: string; expires_at: string; external_id: string; member_resource_role_assignment_id: string; resource_type_id: string; role_id: string; }[]; has_more: boolean; }`\n  Paginated resource role assignments held by one member.\n\n  - `data: { created_at: string; expires_at: string; external_id: string; member_resource_role_assignment_id: string; resource_type_id: string; role_id: string; }[]`\n  - `has_more: boolean`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst response = await client.iam.tenants.members.listResourceRoleAssignments('membership_id', { tenant_id: 'tenant_id' });\n\nconsole.log(response);\n```",
    perLanguage: {
      typescript: {
        method: 'client.iam.tenants.members.listResourceRoleAssignments',
        example:
  ...
  {
    name: 'create',
    endpoint: '/v1/iam/tenants/{tenant_id}/groups',
    httpMethod: 'post',
    summary: 'Create tenant group',
    description: 'Creates an active group with no members in a tenant.',
    stainlessPath: '(resource) iam.tenants.groups > (method) create',
    qualified: 'client.iam.tenants.groups.create',
    params: ['tenant_id: string;', 'actor_user_id: string;', 'name: string;', 'description?: string;'],
    response:
      '{ convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }; group_id: string; }',
    markdown:
      "## create\n\n`client.iam.tenants.groups.create(tenant_id: string, actor_user_id: string, name: string, description?: string): { convex_source_data: object; group_id: string; }`\n\n**post** `/v1/iam/tenants/{tenant_id}/groups`\n\nCreates an active group with no members in a tenant.\n\n### Parameters\n\n- `tenant_id: string`\n\n- `actor_user_id: string`\n  The signed-in end user's ID (their OIDC subject), asserted by the trusted app backend. Used for identity and audit only.\n\n- `name: string`\n  Human-readable group name.\n\n- `description?: string`\n  Optional human-readable group description.\n\n### Returns\n\n- `{ convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }; group_id: string; }`\n  Created tenant group.\n\n  - `convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }`\n  - `group_id: string`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst group = await client.iam.tenants.groups.create('tenant_id', { actor_user_id: 'x', name: 'x' });\n\nconsole.log(group);\n```",
    perLanguage: {
      typescript: {
  ...
  },
  {
    ...
    description: "Lists a tenant's groups, newest first.",
    stainlessPath: '(resource) iam.tenants.groups > (method) list',
    qualified: 'client.iam.tenants.groups.list',
    params: ['tenant_id: string;', 'limit?: number;', 'starting_after?: string;'],
    response:
      "{ data: { archived: boolean; archived_at: string; created_at: string; description: string; group_id: string; name: string; status: 'active' | 'archived'; tenant_id: string; }[]; has_more: boolean; }",
    markdown:
      "## list\n\n`client.iam.tenants.groups.list(tenant_id: string, limit?: number, starting_after?: string): { data: object[]; has_more: boolean; }`\n\n**get** `/v1/iam/tenants/{tenant_id}/groups`\n\nLists a tenant's groups, newest first.\n\n### Parameters\n\n- `tenant_id: string`\n\n- `limit?: number`\n  Maximum number of records to return. Defaults to 50.\n\n- `starting_after?: string`\n  Cursor for forward pagination. Pass the ID of the last item from the previous page.\n\n### Returns\n\n- `{ data: { archived: boolean; archived_at: string; created_at: string; description: string; group_id: string; name: string; status: 'active' | 'archived'; tenant_id: string; }[]; has_more: boolean; }`\n  Paginated tenant groups.\n\n  - `data: { archived: boolean; archived_at: string; created_at: string; description: string; group_id: string; name: string; status: 'active' | 'archived'; tenant_id: string; }[]`\n  - `has_more: boolean`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst groups = await client.iam.tenants.groups.list('tenant_id');\n\nconsole.log(groups);\n```",
    perLanguage: {
      typescript: {
        method: 'client.iam.tenants.groups.list',
        example:
  ...
  },
  {
    ...
    description: 'Returns one tenant group by ID.',
    stainlessPath: '(resource) iam.tenants.groups > (method) get',
    qualified: 'client.iam.tenants.groups.get',
    params: ['tenant_id: string;', 'group_id: string;'],
    response:
      "{ archived: boolean; archived_at: string; created_at: string; description: string; group_id: string; name: string; status: 'active' | 'archived'; tenant_id: string; }",
    markdown:
      "## get\n\n`client.iam.tenants.groups.get(tenant_id: string, group_id: string): { archived: boolean; archived_at: string; created_at: string; description: string; group_id: string; name: string; status: 'active' | 'archived'; tenant_id: string; }`\n\n**get** `/v1/iam/tenants/{tenant_id}/groups/{group_id}`\n\nReturns one tenant group by ID.\n\n### Parameters\n\n- `tenant_id: string`\n\n- `group_id: string`\n\n### Returns\n\n- `{ archived: boolean; archived_at: string; created_at: string; description: string; group_id: string; name: string; status: 'active' | 'archived'; tenant_id: string; }`\n  One tenant group.\n\n  - `archived: boolean`\n  - `archived_at: string`\n  - `created_at: string`\n  - `description: string`\n  - `group_id: string`\n  - `name: string`\n  - `status: 'active' | 'archived'`\n  - `tenant_id: string`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst group = await client.iam.tenants.groups.get('group_id', { tenant_id: 'tenant_id' });\n\nconsole.log(group);\n```",
    perLanguage: {
      typescript: {
        method: 'client.iam.tenants.groups.get',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst group = await client.iam.tenants.groups.get('group_id', { tenant_id: 'tenant_id' });\n\nconsole.log(group.group_id);",
      },
      http: {
  ...
  },
  {
    ...
    description: "Updates a tenant group's name or description.",
    ...
    response:
      '{ convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }; group_id: string; }',
    markdown:
      "## update\n\n`client.iam.tenants.groups.update(tenant_id: string, group_id: string, actor_user_id: string, description?: string, name?: string): { convex_source_data: object; group_id: string; }`\n\n**patch** `/v1/iam/tenants/{tenant_id}/groups/{group_id}`\n\nUpdates a tenant group's name or description.\n\n### Parameters\n\n- `tenant_id: string`\n\n- `group_id: string`\n\n- `actor_user_id: string`\n  The signed-in end user's ID (their OIDC subject), asserted by the trusted app backend. Used for identity and audit only.\n\n- `description?: string`\n  Optional human-readable group description.\n\n- `name?: string`\n  New human-readable group name. Omit to keep the current name.\n\n### Returns\n\n- `{ convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }; group_id: string; }`\n  Result of changing a tenant group.\n\n  - `convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }`\n  - `group_id: string`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst group = await client.iam.tenants.groups.update('group_id', { tenant_id: 'tenant_id', actor_user_id: 'x' });\n\nconsole.log(group);\n```",
    perLanguage: {
      typescript: {
  ...
  },
  {
    ...
    description: 'Permanently deletes a tenant group, cascading its memberships and role assignments.',
    stainlessPath: '(resource) iam.tenants.groups > (method) delete',
    qualified: 'client.iam.tenants.groups.delete',
    params: ['tenant_id: string;', 'group_id: string;', 'actor_user_id?: string;'],
    response:
      '{ convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }; group_id: string; }',
    markdown:
      "## delete\n\n`client.iam.tenants.groups.delete(tenant_id: string, group_id: string, actor_user_id?: string): { convex_source_data: object; group_id: string; }`\n\n**delete** `/v1/iam/tenants/{tenant_id}/groups/{group_id}`\n\nPermanently deletes a tenant group, cascading its memberships and role assignments.\n\n### Parameters\n\n- `tenant_id: string`\n\n- `group_id: string`\n\n- `actor_user_id?: string`\n  The signed-in end user's ID to attribute the operation to that user, or omitted for service authority.\n\n### Returns\n\n- `{ convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }; group_id: string; }`\n  Result of changing a tenant group.\n\n  - `convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }`\n  - `group_id: string`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst group = await client.iam.tenants.groups.delete('group_id', { tenant_id: 'tenant_id' });\n\nconsole.log(group);\n```",
    perLanguage: {
      typescript: {
        method: 'client.iam.tenants.groups.delete',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst group = await client.iam.tenants.groups.delete('group_id', { tenant_id: 'tenant_id' });\n\nconsole.log(group.group_id);",
      },
      http: {
  ...
  },
  {
    ...
    description: 'Archives a tenant group so it stops granting access.',
    ...
    response:
      '{ convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }; group_id: string; }',
    markdown:
      "## archive\n\n`client.iam.tenants.groups.archive(tenant_id: string, group_id: string, actor_user_id: string): { convex_source_data: object; group_id: string; }`\n\n**post** `/v1/iam/tenants/{tenant_id}/groups/{group_id}/archive`\n\nArchives a tenant group so it stops granting access.\n\n### Parameters\n\n- `tenant_id: string`\n\n- `group_id: string`\n\n- `actor_user_id: string`\n  The signed-in end user's ID (their OIDC subject), asserted by the trusted app backend. Used for identity and audit only.\n\n### Returns\n\n- `{ convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }; group_id: string; }`\n  Result of changing a tenant group.\n\n  - `convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }`\n  - `group_id: string`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst response = await client.iam.tenants.groups.archive('group_id', { tenant_id: 'tenant_id', actor_user_id: 'x' });\n\nconsole.log(response);\n```",
    perLanguage: {
      typescript: {
  ...
  },
  {
    ...
    description: 'Restores an archived tenant group so it grants access again.',
    ...
    response:
      '{ convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }; group_id: string; }',
    markdown:
      "## unarchive\n\n`client.iam.tenants.groups.unarchive(tenant_id: string, group_id: string, actor_user_id: string): { convex_source_data: object; group_id: string; }`\n\n**post** `/v1/iam/tenants/{tenant_id}/groups/{group_id}/unarchive`\n\nRestores an archived tenant group so it grants access again.\n\n### Parameters\n\n- `tenant_id: string`\n\n- `group_id: string`\n\n- `actor_user_id: string`\n  The signed-in end user's ID (their OIDC subject), asserted by the trusted app backend. Used for identity and audit only.\n\n### Returns\n\n- `{ convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }; group_id: string; }`\n  Result of changing a tenant group.\n\n  - `convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }`\n  - `group_id: string`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst response = await client.iam.tenants.groups.unarchive('group_id', { tenant_id: 'tenant_id', actor_user_id: 'x' });\n\nconsole.log(response);\n```",
    perLanguage: {
      typescript: {
  ...
  {
    name: 'assign_role',
    endpoint: '/v1/iam/tenants/{tenant_id}/groups/{group_id}/role-assignments',
    httpMethod: 'post',
    summary: 'Assign tenant role to group',
    description: 'Assigns a tenant-wide role to a group.',
    stainlessPath: '(resource) iam.tenants.groups > (method) assign_role',
    qualified: 'client.iam.tenants.groups.assignRole',
    params: [
      'tenant_id: string;',
      'group_id: string;',
      'actor_user_id: string;',
    ...
    response:
      '{ convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }; group_role_assignment_id: string; }',
    markdown:
      "## assign_role\n\n`client.iam.tenants.groups.assignRole(tenant_id: string, group_id: string, actor_user_id: string, role: { id: string; } | { key: string; }, expires_at?: string): { convex_source_data: object; group_role_assignment_id: string; }`\n\n**post** `/v1/iam/tenants/{tenant_id}/groups/{group_id}/role-assignments`\n\nAssigns a tenant-wide role to a group.\n\n### Parameters\n\n- `tenant_id: string`\n\n- `group_id: string`\n\n- `actor_user_id: string`\n  The signed-in end user's ID (their OIDC subject), asserted by the trusted app backend. Used for identity and audit only.\n\n- `role: { id: string; } | { key: string; }`\n  Identifies exactly one IAM role by ID or stable key.\n\n- `expires_at?: string`\n  Assignment expiry. Omit or pass null for a permanent assignment.\n\n### Returns\n\n- `{ convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }; group_role_assignment_id: string; }`\n  Result of changing a group's tenant-wide role assignment.\n\n  - `convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }`\n  - `group_role_assignment_id: string`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst response = await client.iam.tenants.groups.assignRole('group_id', {\n  tenant_id: 'tenant_id',\n  actor_user_id: 'x',\n  role: { id: 'x' },\n});\n\nconsole.log(response);\n```",
    perLanguage: {
      typescript: {
        method: 'client.iam.tenants.groups.assignRole',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst response = await client.iam.tenants.groups.assignRole('group_id', {\n  tenant_id: 'tenant_id',\n  actor_user_id: 'x',\n  role: { id: 'x' },\n});\n\nconsole.log(response.group_role_assignment_id);",
      },
      http: {
        example:
          'curl https://api.hercules.app/v1/iam/tenants/$TENANT_ID/groups/$GROUP_ID/role-assignments \\\n    -H \'Content-Type: application/json\' \\\n    -H "Authorization: Bearer $HERCULES_API_KEY" \\\n    -d \'{\n          "actor_user_id": "x",\n          "role": {\n            "id": "x"\n          }\n        }\'',
      },
    },
  },
  {
    name: 'unassign_role',
    endpoint: '/v1/iam/tenants/{tenant_id}/groups/{group_id}/role-assignments/{assignment_id}',
    httpMethod: 'delete',
    summary: 'Unassign tenant role from group',
    description: 'Removes a tenant-wide role assignment from a group.',
    stainlessPath: '(resource) iam.tenants.groups > (method) unassign_role',
    qualified: 'client.iam.tenants.groups.unassignRole',
    params: ['tenant_id: string;', 'group_id: string;', 'assignment_id: string;', 'actor_user_id?: string;'],
    response:
      '{ convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }; group_role_assignment_id: string; }',
    markdown:
      "## unassign_role\n\n`client.iam.tenants.groups.unassignRole(tenant_id: string, group_id: string, assignment_id: string, actor_user_id?: string): { convex_source_data: object; group_role_assignment_id: string; }`\n\n**delete** `/v1/iam/tenants/{tenant_id}/groups/{group_id}/role-assignments/{assignment_id}`\n\nRemoves a tenant-wide role assignment from a group.\n\n### Parameters\n\n- `tenant_id: string`\n\n- `group_id: string`\n\n- `assignment_id: string`\n\n- `actor_user_id?: string`\n  The signed-in end user's ID to attribute the operation to that user, or omitted for service authority.\n\n### Returns\n\n- `{ convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }; group_role_assignment_id: string; }`\n  Result of changing a group's tenant-wide role assignment.\n\n  - `convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }`\n  - `group_role_assignment_id: string`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst response = await client.iam.tenants.groups.unassignRole('assignment_id', { tenant_id: 'tenant_id', group_id: 'group_id' });\n\nconsole.log(response);\n```",
    perLanguage: {
      typescript: {
        method: 'client.iam.tenants.groups.unassignRole',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst response = await client.iam.tenants.groups.unassignRole('assignment_id', {\n  tenant_id: 'tenant_id',\n  group_id: 'group_id',\n});\n\nconsole.log(response.group_role_assignment_id);",
      },
      http: {
        example:
          'curl https://api.hercules.app/v1/iam/tenants/$TENANT_ID/groups/$GROUP_ID/role-assignments/$ASSIGNMENT_ID \\\n    -X DELETE \\\n    -H "Authorization: Bearer $HERCULES_API_KEY"',
      },
    },
  },
  {
    ...
    endpoint: '/v1/iam/tenants/{tenant_id}/groups/{group_id}/role-assignments',
    httpMethod: 'get',
    summary: "List a group's tenant role assignments",
    description: 'Lists the tenant-wide role assignments held by one group, newest first.',
    stainlessPath: '(resource) iam.tenants.groups > (method) list_role_assignments',
    qualified: 'client.iam.tenants.groups.listRoleAssignments',
    params: ['tenant_id: string;', 'group_id: string;', 'limit?: number;', 'starting_after?: string;'],
    response:
      '{ data: { created_at: string; expires_at: string; group_role_assignment_id: string; role_id: string; }[]; has_more: boolean; }',
    markdown:
      "## list_role_assignments\n\n`client.iam.tenants.groups.listRoleAssignments(tenant_id: string, group_id: string, limit?: number, starting_after?: string): { data: object[]; has_more: boolean; }`\n\n**get** `/v1/iam/tenants/{tenant_id}/groups/{group_id}/role-assignments`\n\nLists the tenant-wide role assignments held by one group, newest first.\n\n### Parameters\n\n- `tenant_id: string`\n\n- `group_id: string`\n\n- `limit?: number`\n  Maximum number of records to return. Defaults to 50.\n\n- `starting_after?: string`\n  Cursor for forward pagination. Pass the ID of the last item from the previous page.\n\n### Returns\n\n- `{ data: { created_at: string; expires_at: string; group_role_assignment_id: string; role_id: string; }[]; has_more: boolean; }`\n  Paginated tenant-wide role assignments held by one group.\n\n  - `data: { created_at: string; expires_at: string; group_role_assignment_id: string; role_id: string; }[]`\n  - `has_more: boolean`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst response = await client.iam.tenants.groups.listRoleAssignments('group_id', { tenant_id: 'tenant_id' });\n\nconsole.log(response);\n```",
    perLanguage: {
      typescript: {
        method: 'client.iam.tenants.groups.listRoleAssignments',
        example:
  ...
  {
    name: 'assign_resource_role',
    endpoint: '/v1/iam/tenants/{tenant_id}/groups/{group_id}/resource-role-assignments',
    httpMethod: 'post',
    summary: 'Assign resource role to group',
    description: 'Assigns a role to a group on one exact resource.',
    stainlessPath: '(resource) iam.tenants.groups > (method) assign_resource_role',
    qualified: 'client.iam.tenants.groups.assignResourceRole',
    params: [
      'tenant_id: string;',
      'group_id: string;',
      'actor_user_id: string;',
    ...
    response:
      '{ convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }; group_resource_role_assignment_id: string; }',
    markdown:
      "## assign_resource_role\n\n`client.iam.tenants.groups.assignResourceRole(tenant_id: string, group_id: string, actor_user_id: string, external_id: string, resource_type: { id: string; } | { key: string; }, role: { id: string; } | { key: string; }, expires_at?: string): { convex_source_data: object; group_resource_role_assignment_id: string; }`\n\n**post** `/v1/iam/tenants/{tenant_id}/groups/{group_id}/resource-role-assignments`\n\nAssigns a role to a group on one exact resource.\n\n### Parameters\n\n- `tenant_id: string`\n\n- `group_id: string`\n\n- `actor_user_id: string`\n  The signed-in end user's ID (their OIDC subject), asserted by the trusted app backend. Used for identity and audit only.\n\n- `external_id: string`\n  The app-defined external ID of the exact resource.\n\n- `resource_type: { id: string; } | { key: string; }`\n  Identifies exactly one IAM resource type by ID or stable key.\n\n- `role: { id: string; } | { key: string; }`\n  Identifies exactly one IAM role by ID or stable key.\n\n- `expires_at?: string`\n  Assignment expiry. Omit or pass null for a permanent assignment.\n\n### Returns\n\n- `{ convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }; group_resource_role_assignment_id: string; }`\n  Result of changing a group resource role assignment.\n\n  - `convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }`\n  - `group_resource_role_assignment_id: string`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst response = await client.iam.tenants.groups.assignResourceRole('group_id', {\n  tenant_id: 'tenant_id',\n  actor_user_id: 'x',\n  external_id: 'x',\n  resource_type: { id: 'x' },\n  role: { id: 'x' },\n});\n\nconsole.log(response);\n```",
    perLanguage: {
      typescript: {
        method: 'client.iam.tenants.groups.assignResourceRole',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst response = await client.iam.tenants.groups.assignResourceRole('group_id', {\n  tenant_id: 'tenant_id',\n  actor_user_id: 'x',\n  external_id: 'x',\n  resource_type: { id: 'x' },\n  role: { id: 'x' },\n});\n\nconsole.log(response.group_resource_role_assignment_id);",
      },
      http: {
        example:
          'curl https://api.hercules.app/v1/iam/tenants/$TENANT_ID/groups/$GROUP_ID/resource-role-assignments \\\n    -H \'Content-Type: application/json\' \\\n    -H "Authorization: Bearer $HERCULES_API_KEY" \\\n    -d \'{\n          "actor_user_id": "x",\n          "external_id": "x",\n          "resource_type": {\n            "id": "x"\n          },\n          "role": {\n            "id": "x"\n          }\n        }\'',
      },
    },
  },
  {
    name: 'unassign_resource_role',
    endpoint: '/v1/iam/tenants/{tenant_id}/groups/{group_id}/resource-role-assignments/{assignment_id}',
    httpMethod: 'delete',
    summary: 'Unassign resource role from group',
    description: 'Removes a group resource role assignment.',
    stainlessPath: '(resource) iam.tenants.groups > (method) unassign_resource_role',
    qualified: 'client.iam.tenants.groups.unassignResourceRole',
    params: ['tenant_id: string;', 'group_id: string;', 'assignment_id: string;', 'actor_user_id?: string;'],
    response:
      '{ convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }; group_resource_role_assignment_id: string; }',
    markdown:
      "## unassign_resource_role\n\n`client.iam.tenants.groups.unassignResourceRole(tenant_id: string, group_id: string, assignment_id: string, actor_user_id?: string): { convex_source_data: object; group_resource_role_assignment_id: string; }`\n\n**delete** `/v1/iam/tenants/{tenant_id}/groups/{group_id}/resource-role-assignments/{assignment_id}`\n\nRemoves a group resource role assignment.\n\n### Parameters\n\n- `tenant_id: string`\n\n- `group_id: string`\n\n- `assignment_id: string`\n\n- `actor_user_id?: string`\n  The signed-in end user's ID to attribute the operation to that user, or omitted for service authority.\n\n### Returns\n\n- `{ convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }; group_resource_role_assignment_id: string; }`\n  Result of changing a group resource role assignment.\n\n  - `convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }`\n  - `group_resource_role_assignment_id: string`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst response = await client.iam.tenants.groups.unassignResourceRole('assignment_id', { tenant_id: 'tenant_id', group_id: 'group_id' });\n\nconsole.log(response);\n```",
    perLanguage: {
      typescript: {
        method: 'client.iam.tenants.groups.unassignResourceRole',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst response = await client.iam.tenants.groups.unassignResourceRole('assignment_id', {\n  tenant_id: 'tenant_id',\n  group_id: 'group_id',\n});\n\nconsole.log(response.group_resource_role_assignment_id);",
      },
      http: {
        example:
          'curl https://api.hercules.app/v1/iam/tenants/$TENANT_ID/groups/$GROUP_ID/resource-role-assignments/$ASSIGNMENT_ID \\\n    -X DELETE \\\n    -H "Authorization: Bearer $HERCULES_API_KEY"',
      },
    },
  },
  {
    ...
    endpoint: '/v1/iam/tenants/{tenant_id}/groups/{group_id}/resource-role-assignments',
    httpMethod: 'get',
    summary: "List a group's resource role assignments",
    description: 'Lists the resource role assignments held by one group, newest first.',
    stainlessPath: '(resource) iam.tenants.groups > (method) list_resource_role_assignments',
    qualified: 'client.iam.tenants.groups.listResourceRoleAssignments',
    params: ['tenant_id: string;', 'group_id: string;', 'limit?: number;', 'starting_after?: string;'],
    response:
      '{ data: { created_at: string; expires_at: string; external_id: string; group_resource_role_assignment_id: string; resource_type_id: string; role_id: string; }[]; has_more: boolean; }',
    markdown:
      "## list_resource_role_assignments\n\n`client.iam.tenants.groups.listResourceRoleAssignments(tenant_id: string, group_id: string, limit?: number, starting_after?: string): { data: object[]; has_more: boolean; }`\n\n**get** `/v1/iam/tenants/{tenant_id}/groups/{group_id}/resource-role-assignments`\n\nLists the resource role assignments held by one group, newest first.\n\n### Parameters\n\n- `tenant_id: string`\n\n- `group_id: string`\n\n- `limit?: number`\n  Maximum number of records to return. Defaults to 50.\n\n- `starting_after?: string`\n  Cursor for forward pagination. Pass the ID of the last item from the previous page.\n\n### Returns\n\n- `{ data: { created_at: string; expires_at: string; external_id: string; group_resource_role_assignment_id: string; resource_type_id: string; role_id: string; }[]; has_more: boolean; }`\n  Paginated resource role assignments held by one group.\n\n  - `data: { created_at: string; expires_at: string; external_id: string; group_resource_role_assignment_id: string; resource_type_id: string; role_id: string; }[]`\n  - `has_more: boolean`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst response = await client.iam.tenants.groups.listResourceRoleAssignments('group_id', { tenant_id: 'tenant_id' });\n\nconsole.log(response);\n```",
    perLanguage: {
      typescript: {
        method: 'client.iam.tenants.groups.listResourceRoleAssignments',
        example:
  ...
  {
    name: 'add',
    endpoint: '/v1/iam/tenants/{tenant_id}/groups/{group_id}/members/{membership_id}',
    httpMethod: 'put',
    summary: 'Add tenant group member',
    description: 'Adds a tenant member to a group.',
    stainlessPath: '(resource) iam.tenants.groups.members > (method) add',
    qualified: 'client.iam.tenants.groups.members.add',
    params: ['tenant_id: string;', 'group_id: string;', 'membership_id: string;', 'actor_user_id: string;'],
    response:
      '{ convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }; group_id: string; membership_id: string; }',
    markdown:
      "## add\n\n`client.iam.tenants.groups.members.add(tenant_id: string, group_id: string, membership_id: string, actor_user_id: string): { convex_source_data: object; group_id: string; membership_id: string; }`\n\n**put** `/v1/iam/tenants/{tenant_id}/groups/{group_id}/members/{membership_id}`\n\nAdds a tenant member to a group.\n\n### Parameters\n\n- `tenant_id: string`\n\n- `group_id: string`\n\n- `membership_id: string`\n\n- `actor_user_id: string`\n  The signed-in end user's ID (their OIDC subject), asserted by the trusted app backend. Used for identity and audit only.\n\n### Returns\n\n- `{ convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }; group_id: string; membership_id: string; }`\n  Result of changing a tenant group membership.\n\n  - `convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }`\n  - `group_id: string`\n  - `membership_id: string`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst response = await client.iam.tenants.groups.members.add('membership_id', {\n  tenant_id: 'tenant_id',\n  group_id: 'group_id',\n  actor_user_id: 'x',\n});\n\nconsole.log(response);\n```",
    perLanguage: {
      typescript: {
      ...
      http: {
        example:
          'curl https://api.hercules.app/v1/iam/tenants/$TENANT_ID/groups/$GROUP_ID/members/$MEMBERSHIP_ID \\\n    -X PUT \\\n    -H \'Content-Type: application/json\' \\\n    -H "Authorization: Bearer $HERCULES_API_KEY" \\\n    -d \'{\n          "actor_user_id": "x"\n        }\'',
      },
    },
  },
  {
    name: 'list',
    endpoint: '/v1/iam/tenants/{tenant_id}/groups/{group_id}/members',
    httpMethod: 'get',
    summary: "List a group's members",
    description: 'Lists the active members of one tenant group, newest first.',
    stainlessPath: '(resource) iam.tenants.groups.members > (method) list',
    qualified: 'client.iam.tenants.groups.members.list',
    params: ['tenant_id: string;', 'group_id: string;', 'limit?: number;', 'starting_after?: string;'],
    response:
      "{ data: { created_at: string; group_membership_id: string; membership_id: string; source: 'system' | 'manual'; status: 'active' | 'blocked' | 'suspended' | 'pending_approval' | 'removed'; user_email: string; user_id: string; user_image: string; user_name: string; }[]; has_more: boolean; }",
    markdown:
      "## list\n\n`client.iam.tenants.groups.members.list(tenant_id: string, group_id: string, limit?: number, starting_after?: string): { data: object[]; has_more: boolean; }`\n\n**get** `/v1/iam/tenants/{tenant_id}/groups/{group_id}/members`\n\nLists the active members of one tenant group, newest first.\n\n### Parameters\n\n- `tenant_id: string`\n\n- `group_id: string`\n\n- `limit?: number`\n  Maximum number of records to return. Defaults to 50.\n\n- `starting_after?: string`\n  Cursor for forward pagination. Pass the ID of the last item from the previous page.\n\n### Returns\n\n- `{ data: { created_at: string; group_membership_id: string; membership_id: string; source: 'system' | 'manual'; status: 'active' | 'blocked' | 'suspended' | 'pending_approval' | 'removed'; user_email: string; user_id: string; user_image: string; user_name: string; }[]; has_more: boolean; }`\n  Paginated members of a tenant group.\n\n  - `data: { created_at: string; group_membership_id: string; membership_id: string; source: 'system' | 'manual'; status: 'active' | 'blocked' | 'suspended' | 'pending_approval' | 'removed'; user_email: string; user_id: string; user_image: string; user_name: string; }[]`\n  - `has_more: boolean`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst members = await client.iam.tenants.groups.members.list('group_id', { tenant_id: 'tenant_id' });\n\nconsole.log(members);\n```",
    perLanguage: {
      typescript: {
        method: 'client.iam.tenants.groups.members.list',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst members = await client.iam.tenants.groups.members.list('group_id', {\n  tenant_id: 'tenant_id',\n});\n\nconsole.log(members.data);",
      },
      http: {
        example:
          'curl https://api.hercules.app/v1/iam/tenants/$TENANT_ID/groups/$GROUP_ID/members \\\n    -H "Authorization: Bearer $HERCULES_API_KEY"',
      },
    },
  },
  {
    ...
    endpoint: '/v1/iam/tenants/{tenant_id}/groups/{group_id}/members/{membership_id}',
    httpMethod: 'delete',
    summary: 'Remove tenant group member',
    description: 'Removes a tenant member from a group.',
    ...
    response:
      '{ convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }; group_id: string; membership_id: string; }',
    markdown:
      "## remove\n\n`client.iam.tenants.groups.members.remove(tenant_id: string, group_id: string, membership_id: string, actor_user_id?: string): { convex_source_data: object; group_id: string; membership_id: string; }`\n\n**delete** `/v1/iam/tenants/{tenant_id}/groups/{group_id}/members/{membership_id}`\n\nRemoves a tenant member from a group.\n\n### Parameters\n\n- `tenant_id: string`\n\n- `group_id: string`\n\n- `membership_id: string`\n\n- `actor_user_id?: string`\n  The signed-in end user's ID to attribute the operation to that user, or omitted for service authority.\n\n### Returns\n\n- `{ convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }; group_id: string; membership_id: string; }`\n  Result of changing a tenant group membership.\n\n  - `convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }`\n  - `group_id: string`\n  - `membership_id: string`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst member = await client.iam.tenants.groups.members.remove('membership_id', { tenant_id: 'tenant_id', group_id: 'group_id' });\n\nconsole.log(member);\n```",
    perLanguage: {
      typescript: {
        method: 'client.iam.tenants.groups.members.remove',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst member = await client.iam.tenants.groups.members.remove('membership_id', {\n  tenant_id: 'tenant_id',\n  group_id: 'group_id',\n});\n\nconsole.log(member.group_id);",
      },
      http: {
        example:
          'curl https://api.hercules.app/v1/iam/tenants/$TENANT_ID/groups/$GROUP_ID/members/$MEMBERSHIP_ID \\\n    -X DELETE \\\n    -H "Authorization: Bearer $HERCULES_API_KEY"',
  ...
  {
    name: 'create',
    endpoint: '/v1/iam/tenants/{tenant_id}/roles',
    httpMethod: 'post',
    summary: 'Create role',
    description:
      'Creates a tenant-scoped role with a permission set. Shared and app-scoped roles are managed via iam.jsonc, not this API.',
    stainlessPath: '(resource) iam.tenants.roles > (method) create',
    qualified: 'client.iam.tenants.roles.create',
    params: [
      'tenant_id: string;',
      'actor_user_id: string;',
      'key: string;',
    ...
    response:
      '{ convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }; is_app_scope: boolean; role_id: string; }',
    markdown:
      "## create\n\n`client.iam.tenants.roles.create(tenant_id: string, actor_user_id: string, key: string, name: string, permission_keys: string[], description?: string): { convex_source_data: object; is_app_scope: boolean; role_id: string; }`\n\n**post** `/v1/iam/tenants/{tenant_id}/roles`\n\nCreates a tenant-scoped role with a permission set. Shared and app-scoped roles are managed via iam.jsonc, not this API.\n\n### Parameters\n\n- `tenant_id: string`\n\n- `actor_user_id: string`\n  The signed-in end user's ID (their OIDC subject), asserted by the trusted app backend. Used for identity and audit only.\n\n- `key: string`\n  Stable, unique key for the role within its scope.\n\n- `name: string`\n  Human-readable role name.\n\n- `permission_keys: string[]`\n  Permission keys from the deployment's IAM catalog granted by this role.\n\n- `description?: string`\n  Optional human-readable role description.\n\n### Returns\n\n- `{ convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }; is_app_scope: boolean; role_id: string; }`\n  Created IAM role.\n\n  - `convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }`\n  - `is_app_scope: boolean`\n  - `role_id: string`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst role = await client.iam.tenants.roles.create('tenant_id', {\n  actor_user_id: 'x',\n  key: 'x',\n  name: 'x',\n  permission_keys: ['x'],\n});\n\nconsole.log(role);\n```",
    perLanguage: {
      typescript: {
  ...
  },
  {
    ...
    stainlessPath: '(resource) iam.tenants.roles > (method) list',
    qualified: 'client.iam.tenants.roles.list',
    params: ['tenant_id: string;', 'key?: string;', 'limit?: number;', 'starting_after?: string;'],
    response:
      '{ data: { description: string; is_app_scope: boolean; key: string; name: string; permissions: { key: string; permission_id: string; }[]; role_id: string; tenant_id: string; }[]; has_more: boolean; }',
    markdown:
      "## list\n\n`client.iam.tenants.roles.list(tenant_id: string, key?: string, limit?: number, starting_after?: string): { data: object[]; has_more: boolean; }`\n\n**get** `/v1/iam/tenants/{tenant_id}/roles`\n\nLists the roles visible to a tenant: its tenant-scoped roles plus the deployment's shared and app-scoped roles. Pass a key to fetch one role by its stable key.\n\n### Parameters\n\n- `tenant_id: string`\n\n- `key?: string`\n  Return only the role with this stable key, resolved within the tenant scope.\n\n- `limit?: number`\n  Maximum number of records to return. Defaults to 50.\n\n- `starting_after?: string`\n  Cursor for forward pagination. Pass the ID of the last item from the previous page.\n\n### Returns\n\n- `{ data: { description: string; is_app_scope: boolean; key: string; name: string; permissions: { key: string; permission_id: string; }[]; role_id: string; tenant_id: string; }[]; has_more: boolean; }`\n  Paginated IAM roles.\n\n  - `data: { description: string; is_app_scope: boolean; key: string; name: string; permissions: { key: string; permission_id: string; }[]; role_id: string; tenant_id: string; }[]`\n  - `has_more: boolean`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst roles = await client.iam.tenants.roles.list('tenant_id');\n\nconsole.log(roles);\n```",
    perLanguage: {
      typescript: {
        method: 'client.iam.tenants.roles.list',
        example:
  ...
  },
  {
    ...
    description: 'Returns one IAM role by ID, with its full permission set.',
    stainlessPath: '(resource) iam.tenants.roles > (method) get',
    qualified: 'client.iam.tenants.roles.get',
    params: ['tenant_id: string;', 'role_id: string;'],
    response:
      '{ description: string; is_app_scope: boolean; key: string; name: string; permissions: { description: string; is_app_scope: boolean; key: string; name: string; permission_id: string; }[]; role_id: string; tenant_id: string; }',
    markdown:
      "## get\n\n`client.iam.tenants.roles.get(tenant_id: string, role_id: string): { description: string; is_app_scope: boolean; key: string; name: string; permissions: object[]; role_id: string; tenant_id: string; }`\n\n**get** `/v1/iam/tenants/{tenant_id}/roles/{role_id}`\n\nReturns one IAM role by ID, with its full permission set.\n\n### Parameters\n\n- `tenant_id: string`\n\n- `role_id: string`\n\n### Returns\n\n- `{ description: string; is_app_scope: boolean; key: string; name: string; permissions: { description: string; is_app_scope: boolean; key: string; name: string; permission_id: string; }[]; role_id: string; tenant_id: string; }`\n  One IAM role with full permission details.\n\n  - `description: string`\n  - `is_app_scope: boolean`\n  - `key: string`\n  - `name: string`\n  - `permissions: { description: string; is_app_scope: boolean; key: string; name: string; permission_id: string; }[]`\n  - `role_id: string`\n  - `tenant_id: string`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst role = await client.iam.tenants.roles.get('role_id', { tenant_id: 'tenant_id' });\n\nconsole.log(role);\n```",
    perLanguage: {
      typescript: {
        method: 'client.iam.tenants.roles.get',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst role = await client.iam.tenants.roles.get('role_id', { tenant_id: 'tenant_id' });\n\nconsole.log(role.role_id);",
      },
      http: {
  ...
  },
  {
    ...
    response:
      '{ convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }; is_app_scope: boolean; role_id: string; }',
    markdown:
      "## update\n\n`client.iam.tenants.roles.update(tenant_id: string, role_id: string, actor_user_id: string, description?: string, name?: string, permission_keys?: string[]): { convex_source_data: object; is_app_scope: boolean; role_id: string; }`\n\n**patch** `/v1/iam/tenants/{tenant_id}/roles/{role_id}`\n\nUpdates a tenant-scoped role's name, description, or permission set. Shared and app-scoped roles are managed via iam.jsonc, not this API.\n\n### Parameters\n\n- `tenant_id: string`\n\n- `role_id: string`\n\n- `actor_user_id: string`\n  The signed-in end user's ID (their OIDC subject), asserted by the trusted app backend. Used for identity and audit only.\n\n- `description?: string`\n  Optional human-readable role description.\n\n- `name?: string`\n  New human-readable role name.\n\n- `permission_keys?: string[]`\n  Replacement permission set. Omit to leave the role's permissions unchanged.\n\n### Returns\n\n- `{ convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }; is_app_scope: boolean; role_id: string; }`\n  Result of changing an IAM role.\n\n  - `convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }`\n  - `is_app_scope: boolean`\n  - `role_id: string`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst role = await client.iam.tenants.roles.update('role_id', { tenant_id: 'tenant_id', actor_user_id: 'x' });\n\nconsole.log(role);\n```",
    perLanguage: {
      typescript: {
  ...
  },
  {
    ...
    stainlessPath: '(resource) iam.tenants.roles > (method) delete',
    qualified: 'client.iam.tenants.roles.delete',
    params: ['tenant_id: string;', 'role_id: string;', 'actor_user_id?: string;'],
    response:
      '{ convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }; role_id: string; }',
    markdown:
      "## delete\n\n`client.iam.tenants.roles.delete(tenant_id: string, role_id: string, actor_user_id?: string): { convex_source_data: object; role_id: string; }`\n\n**delete** `/v1/iam/tenants/{tenant_id}/roles/{role_id}`\n\nPermanently deletes a tenant-scoped role, cascading its permissions and assignments. Shared and app-scoped roles are managed via iam.jsonc, not this API.\n\n### Parameters\n\n- `tenant_id: string`\n\n- `role_id: string`\n\n- `actor_user_id?: string`\n  The signed-in end user's ID to attribute the operation to that user, or omitted for service authority.\n\n### Returns\n\n- `{ convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }; role_id: string; }`\n  Deleted IAM role.\n\n  - `convex_source_data: { changed: boolean; projection_ids: string[]; source_version: number; }`\n  - `role_id: string`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst role = await client.iam.tenants.roles.delete('role_id', { tenant_id: 'tenant_id' });\n\nconsole.log(role);\n```",
    perLanguage: {
      typescript: {
        method: 'client.iam.tenants.roles.delete',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst role = await client.iam.tenants.roles.delete('role_id', { tenant_id: 'tenant_id' });\n\nconsole.log(role.role_id);",
      },
      http: {
  ...
  {
    name: 'list',
    endpoint: '/v1/iam/tenants/{tenant_id}/access-rules',
    httpMethod: 'get',
    summary: 'List tenant access rules',
    description:
      'Lists email and domain rules that allow or deny entry to a tenant. Active rules are returned by default.',
    stainlessPath: '(resource) iam.tenants.access_rules > (method) list',
    qualified: 'client.iam.tenants.accessRules.list',
    params: [
      'tenant_id: string;',
      "effect?: 'allow' | 'deny';",
    ...
    response:
      "{ data: { archived: boolean; archived_at: string; created_at: string; effect: 'allow' | 'deny'; reason: string; rule_id: string; subject: { type: 'email'; value: string; } | { type: 'domain'; value: string; }; }[]; has_more: boolean; }",
    markdown:
      "## list\n\n`client.iam.tenants.accessRules.list(tenant_id: string, effect?: 'allow' | 'deny', include_archived?: string, limit?: number, starting_after?: string, subject_type?: 'email' | 'domain'): { data: object[]; has_more: boolean; }`\n\n**get** `/v1/iam/tenants/{tenant_id}/access-rules`\n\nLists email and domain rules that allow or deny entry to a tenant. Active rules are returned by default.\n\n### Parameters\n\n- `tenant_id: string`\n\n- `effect?: 'allow' | 'deny'`\n  Filter by rule effect.\n\n- `include_archived?: string`\n  Whether to include archived rules. Defaults to active rules only.\n\n- `limit?: number`\n  Maximum number of records to return. Defaults to 50.\n\n- `starting_after?: string`\n  Cursor for forward pagination. Pass the ID of the last item from the previous page.\n\n- `subject_type?: 'email' | 'domain'`\n  Filter by subject type.\n\n### Returns\n\n- `{ data: { archived: boolean; archived_at: string; created_at: string; effect: 'allow' | 'deny'; reason: string; rule_id: string; subject: { type: 'email'; value: string; } | { type: 'domain'; value: string; }; }[]; has_more: boolean; }`\n  Paginated tenant access rules.\n\n  - `data: { archived: boolean; archived_at: string; created_at: string; effect: 'allow' | 'deny'; reason: string; rule_id: string; subject: { type: 'email'; value: string; } | { type: 'domain'; value: string; }; }[]`\n  - `has_more: boolean`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst accessRules = await client.iam.tenants.accessRules.list('tenant_id');\n\nconsole.log(accessRules);\n```",
    perLanguage: {
      typescript: {
      ...
      http: {
        example:
          'curl https://api.hercules.app/v1/iam/tenants/$TENANT_ID/access-rules \\\n    -H "Authorization: Bearer $HERCULES_API_KEY"',
      },
    },
  },
  {
    ...
    endpoint: '/v1/iam/tenants/{tenant_id}/access-rules',
    httpMethod: 'post',
    summary: 'Create tenant access rule',
    description:
      'Creates an allow or deny rule for an email address or domain. The rule takes effect immediately for matching users.',
    stainlessPath: '(resource) iam.tenants.access_rules > (method) create',
    qualified: 'client.iam.tenants.accessRules.create',
    params: [
      'tenant_id: string;',
      'actor_user_id: string;',
      "effect: 'allow' | 'deny';",
    ...
    markdown:
      "## create\n\n`client.iam.tenants.accessRules.create(tenant_id: string, actor_user_id: string, effect: 'allow' | 'deny', subject: { type: 'email'; value: string; } | { type: 'domain'; value: string; }, reason?: string): { rule_id: string; }`\n\n**post** `/v1/iam/tenants/{tenant_id}/access-rules`\n\nCreates an allow or deny rule for an email address or domain. The rule takes effect immediately for matching users.\n\n### Parameters\n\n- `tenant_id: string`\n\n- `actor_user_id: string`\n  The signed-in end user's ID (their OIDC subject), asserted by the trusted app backend. Used for identity and audit only.\n\n- `effect: 'allow' | 'deny'`\n  Whether matching users are allowed or denied entry.\n\n- `subject: { type: 'email'; value: string; } | { type: 'domain'; value: string; }`\n  Email or domain matched by the rule.\n\n- `reason?: string`\n  Optional administrative reason.\n\n### Returns\n\n- `{ rule_id: string; }`\n  Created tenant access rule.\n\n  - `rule_id: string`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst accessRule = await client.iam.tenants.accessRules.create('tenant_id', {\n  actor_user_id: 'x',\n  effect: 'allow',\n  subject: { type: 'email', value: 'dev@stainless.com' },\n});\n\nconsole.log(accessRule);\n```",
    perLanguage: {
      typescript: {
        method: 'client.iam.tenants.accessRules.create',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst accessRule = await client.iam.tenants.accessRules.create('tenant_id', {\n  actor_user_id: 'x',\n  effect: 'allow',\n  subject: { type: 'email', value: 'dev@stainless.com' },\n});\n\nconsole.log(accessRule.rule_id);",
      },
      http: {
        example:
  ...
  },
  {
    ...
    description: 'Archives an access rule so it no longer affects who can enter the tenant.',
    stainlessPath: '(resource) iam.tenants.access_rules > (method) archive',
    qualified: 'client.iam.tenants.accessRules.archive',
    params: ['tenant_id: string;', 'rule_id: string;', 'actor_user_id: string;'],
    response: '{ archived: boolean; rule_id: string; }',
    markdown:
      "## archive\n\n`client.iam.tenants.accessRules.archive(tenant_id: string, rule_id: string, actor_user_id: string): { archived: boolean; rule_id: string; }`\n\n**post** `/v1/iam/tenants/{tenant_id}/access-rules/{rule_id}/archive`\n\nArchives an access rule so it no longer affects who can enter the tenant.\n\n### Parameters\n\n- `tenant_id: string`\n\n- `rule_id: string`\n\n- `actor_user_id: string`\n  The signed-in end user's ID (their OIDC subject), asserted by the trusted app backend. Used for identity and audit only.\n\n### Returns\n\n- `{ archived: boolean; rule_id: string; }`\n  Archived tenant access rule.\n\n  - `archived: boolean`\n  - `rule_id: string`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst response = await client.iam.tenants.accessRules.archive('rule_id', { tenant_id: 'tenant_id', actor_user_id: 'x' });\n\nconsole.log(response);\n```",
    perLanguage: {
      typescript: {
        method: 'client.iam.tenants.accessRules.archive',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst response = await client.iam.tenants.accessRules.archive('rule_id', {\n  tenant_id: 'tenant_id',\n  actor_user_id: 'x',\n});\n\nconsole.log(response.rule_id);",
      },
      http: {
        example:
  ...
  {
    name: 'list',
    endpoint: '/v1/iam/tenants/{tenant_id}/audit-events',
    httpMethod: 'get',
    summary: 'List tenant audit events',
    description: 'Lists IAM audit events for a tenant, newest first.',
    stainlessPath: '(resource) iam.tenants.audit_events > (method) list',
    qualified: 'client.iam.tenants.auditEvents.list',
    params: ['tenant_id: string;', 'before?: string;', 'limit?: number;', 'starting_after?: string;'],
    response:
      "{ data: { action: string; actor_type: 'system' | 'platform_user' | 'app_user' | 'agent' | 'service'; audit_event_id: string; created_at: string; metadata: object; outcome: 'success' | 'denied' | 'failure'; reason_code: string; target_id: string; target_type: string; tenant_id: string; }[]; has_more: boolean; }",
    markdown:
      "## list\n\n`client.iam.tenants.auditEvents.list(tenant_id: string, before?: string, limit?: number, starting_after?: string): { data: object[]; has_more: boolean; }`\n\n**get** `/v1/iam/tenants/{tenant_id}/audit-events`\n\nLists IAM audit events for a tenant, newest first.\n\n### Parameters\n\n- `tenant_id: string`\n\n- `before?: string`\n  Return events strictly before this timestamp.\n\n- `limit?: number`\n  Maximum number of records to return. Defaults to 50.\n\n- `starting_after?: string`\n  Cursor for forward pagination. Pass the ID of the last item from the previous page.\n\n### Returns\n\n- `{ data: { action: string; actor_type: 'system' | 'platform_user' | 'app_user' | 'agent' | 'service'; audit_event_id: string; created_at: string; metadata: object; outcome: 'success' | 'denied' | 'failure'; reason_code: string; target_id: string; target_type: string; tenant_id: string; }[]; has_more: boolean; }`\n  Paginated tenant IAM audit events.\n\n  - `data: { action: string; actor_type: 'system' | 'platform_user' | 'app_user' | 'agent' | 'service'; audit_event_id: string; created_at: string; metadata: object; outcome: 'success' | 'denied' | 'failure'; reason_code: string; target_id: string; target_type: string; tenant_id: string; }[]`\n  - `has_more: boolean`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst auditEvents = await client.iam.tenants.auditEvents.list('tenant_id');\n\nconsole.log(auditEvents);\n```",
    perLanguage: {
      typescript: {
      ...
      http: {
        example:
          'curl https://api.hercules.app/v1/iam/tenants/$TENANT_ID/audit-events \\\n    -H "Authorization: Bearer $HERCULES_API_KEY"',
      },
    },
  },
  {
    ...
    description: 'Lists active, unexpired invitation links in a tenant.',
    stainlessPath: '(resource) iam.tenants.invitations > (method) list',
    qualified: 'client.iam.tenants.invitations.list',
    params: ['tenant_id: string;', 'limit?: number;', 'starting_after?: string;'],
    response:
      "{ data: { constraint: { type: 'email'; value: string; } | { type: 'domain'; value: string; }; created_at: string; delivery: { to_emails: string[]; from_email?: string; }; expires_at: string; invitation_id: string; link: string; max_uses: number; role_ids: string[]; tenant_id: string; use_count: number; }[]; has_more: boolean; }",
    markdown:
      "## list\n\n`client.iam.tenants.invitations.list(tenant_id: string, limit?: number, starting_after?: string): { data: object[]; has_more: boolean; }`\n\n**get** `/v1/iam/tenants/{tenant_id}/invitations`\n\nLists active, unexpired invitation links in a tenant.\n\n### Parameters\n\n- `tenant_id: string`\n\n- `limit?: number`\n  Maximum number of records to return. Defaults to 50.\n\n- `starting_after?: string`\n  Cursor for forward pagination. Pass the ID of the last item from the previous page.\n\n### Returns\n\n- `{ data: { constraint: { type: 'email'; value: string; } | { type: 'domain'; value: string; }; created_at: string; delivery: { to_emails: string[]; from_email?: string; }; expires_at: string; invitation_id: string; link: string; max_uses: number; role_ids: string[]; tenant_id: string; use_count: number; }[]; has_more: boolean; }`\n  Paginated active invitation links in one tenant.\n\n  - `data: { constraint: { type: 'email'; value: string; } | { type: 'domain'; value: string; }; created_at: string; delivery: { to_emails: string[]; from_email?: string; }; expires_at: string; invitation_id: string; link: string; max_uses: number; role_ids: string[]; tenant_id: string; use_count: number; }[]`\n  - `has_more: boolean`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst invitations = await client.iam.tenants.invitations.list('tenant_id');\n\nconsole.log(invitations);\n```",
    perLanguage: {
      typescript: {
  ...
  {
    name: 'revoke',
    endpoint: '/v1/iam/tenants/{tenant_id}/invitations/{invitation_id}',
    httpMethod: 'delete',
    ...
    params: ['tenant_id: string;', 'invitation_id: string;', 'actor_user_id?: string;'],
    response: '{ invitation_id: string; revoked: boolean; }',
    markdown:
      "## revoke\n\n`client.iam.tenants.invitations.revoke(tenant_id: string, invitation_id: string, actor_user_id?: string): { invitation_id: string; revoked: boolean; }`\n\n**delete** `/v1/iam/tenants/{tenant_id}/invitations/{invitation_id}`\n\nRevokes a pending invitation. Access already granted by an accepted invitation is unaffected.\n\n### Parameters\n\n- `tenant_id: string`\n\n- `invitation_id: string`\n\n- `actor_user_id?: string`\n  The signed-in end user's ID to attribute the operation to that user, or omitted for service authority.\n\n### Returns\n\n- `{ invitation_id: string; revoked: boolean; }`\n  Revoked invitation.\n\n  - `invitation_id: string`\n  - `revoked: boolean`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst response = await client.iam.tenants.invitations.revoke('invitation_id', { tenant_id: 'tenant_id' });\n\nconsole.log(response);\n```",
    perLanguage: {
      typescript: {
  ...
  },
  {
    ...
    response:
      "{ id: string; cancellation_timing: 'immediate' | 'at_billing_period_end'; environment: 'sandbox' | 'live'; status: string; canceled_at?: string; }",
    markdown:
      "## cancel\n\n`client.commerce.cancel(customer_id: string, subscription_id: string, cancellation_timing?: 'immediate' | 'at_billing_period_end'): { id: string; cancellation_timing: 'immediate' | 'at_billing_period_end'; environment: 'sandbox' | 'live'; status: string; canceled_at?: string; }`\n\n**post** `/v1/commerce/cancel`\n\nCancels a customer's subscription. By default, the subscription remains active until the end of the current billing period. Set cancellation_timing to 'immediate' to cancel immediately.\n\n### Parameters\n\n- `customer_id: string`\n  The customer ID\n\n- `subscription_id: string`\n  The subscription ID to cancel\n\n- `cancellation_timing?: 'immediate' | 'at_billing_period_end'`\n  When to cancel the subscription. Defaults to 'at_billing_period_end' to allow customers to use remaining time.\n\n### Returns\n\n- `{ id: string; cancellation_timing: 'immediate' | 'at_billing_period_end'; environment: 'sandbox' | 'live'; status: string; canceled_at?: string; }`\n  Cancel subscription response\n\n  - `id: string`\n  - `cancellation_timing: 'immediate' | 'at_billing_period_end'`\n  - `environment: 'sandbox' | 'live'`\n  - `status: string`\n  - `canceled_at?: string`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst response = await client.commerce.cancel({ customer_id: 'cus_1234567890', subscription_id: 'sub_1234567890' });\n\nconsole.log(response);\n```",
    perLanguage: {
      typescript: {
        method: 'client.commerce.cancel',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst response = await client.commerce.cancel({\n  customer_id: 'cus_1234567890',\n  subscription_id: 'sub_1234567890',\n});\n\nconsole.log(response.id);",
      },
      http: {
        example:
          'curl https://api.hercules.app/v1/commerce/cancel \\\n    -H \'Content-Type: application/json\' \\\n    -H "Authorization: Bearer $HERCULES_API_KEY" \\\n    -d \'{\n          "customer_id": "cus_1234567890",\n          "subscription_id": "sub_1234567890"\n        }\'',
      },
    },
  },
  {
    ...
    response: "{ environment: 'sandbox' | 'live'; has_access: boolean; }",
    markdown:
      "## check\n\n`client.commerce.check(customer_id: string, resource_id: string): { environment: 'sandbox' | 'live'; has_access: boolean; }`\n\n**post** `/v1/commerce/check`\n\nVerifies if a customer has access to a specific feature. Use this to gate features in your app based on the customer's active subscription and the entitlements attached to their product. Hercules recommends calling this before allowing access to premium features.\n\n### Parameters\n\n- `customer_id: string`\n  The customer ID\n\n- `resource_id: string`\n  The resource ID to check for access\n\n### Returns\n\n- `{ environment: 'sandbox' | 'live'; has_access: boolean; }`\n  Check resource access response.\n\n  - `environment: 'sandbox' | 'live'`\n  - `has_access: boolean`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst response = await client.commerce.check({ customer_id: 'cus_1234567890', resource_id: 'feat_1234567890' });\n\nconsole.log(response);\n```",
    perLanguage: {
      typescript: {
        method: 'client.commerce.check',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst response = await client.commerce.check({\n  customer_id: 'cus_1234567890',\n  resource_id: 'feat_1234567890',\n});\n\nconsole.log(response.environment);",
      },
      http: {
        example:
          'curl https://api.hercules.app/v1/commerce/check \\\n    -H \'Content-Type: application/json\' \\\n    -H "Authorization: Bearer $HERCULES_API_KEY" \\\n    -d \'{\n          "customer_id": "cus_1234567890",\n          "resource_id": "feat_1234567890"\n        }\'',
      },
    },
  },
  {
  ...
  },
  {
    ...
    response:
      "{ id: string; created: string; environment: 'sandbox' | 'live'; address?: { city?: string; country?: string; line1?: string; line2?: string; postal_code?: string; state?: string; }; email?: string; name?: string; phone?: string; }",
    markdown:
      "## get\n\n`client.commerce.customers.get(customer_id: string): { id: string; created: string; environment: 'sandbox' | 'live'; address?: customer_address; email?: string; name?: string; phone?: string; }`\n\n**get** `/v1/commerce/customers/{customer_id}`\n\nRetrieves a customer by ID. Returns the customer object including contact information and billing address.\n\n### Parameters\n\n- `customer_id: string`\n\n### Returns\n\n- `{ id: string; created: string; environment: 'sandbox' | 'live'; address?: { city?: string; country?: string; line1?: string; line2?: string; postal_code?: string; state?: string; }; email?: string; name?: string; phone?: string; }`\n  A billable customer with detailed subscription, invoice, and payment method information.\n\n  - `id: string`\n  - `created: string`\n  - `environment: 'sandbox' | 'live'`\n  - `address?: { city?: string; country?: string; line1?: string; line2?: string; postal_code?: string; state?: string; }`\n  - `email?: string`\n  - `name?: string`\n  - `phone?: string`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst customer = await client.commerce.customers.get('cus_1234567890');\n\nconsole.log(customer);\n```",
    perLanguage: {
      typescript: {
        method: 'client.commerce.customers.get',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst customer = await client.commerce.customers.get('cus_1234567890');\n\nconsole.log(customer.id);",
      },
      http: {
        example:
          'curl https://api.hercules.app/v1/commerce/customers/$CUSTOMER_ID \\\n    -H "Authorization: Bearer $HERCULES_API_KEY"',
      },
    },
  },
  {
    ...
    response:
      "{ id: string; created: string; environment: 'sandbox' | 'live'; address?: { city?: string; country?: string; line1?: string; line2?: string; postal_code?: string; state?: string; }; email?: string; name?: string; phone?: string; }",
    markdown:
      "## create\n\n`client.commerce.customers.create(id?: string, address?: { city?: string; country?: string; line1?: string; line2?: string; postal_code?: string; state?: string; }, email?: string, name?: string, phone?: string): { id: string; created: string; environment: 'sandbox' | 'live'; address?: customer_address; email?: string; name?: string; phone?: string; }`\n\n**post** `/v1/commerce/customers`\n\nCreates a new billable customer. A customer represents the entity in your app that will be charged—typically a user, organization, or project. Hercules recommends creating a customer immediately after creating the corresponding entity in your app.\n\n### Parameters\n\n- `id?: string`\n  Optional custom ID for the customer. Must start with 'cus_'. If not provided, one will be generated.\n\n- `address?: { city?: string; country?: string; line1?: string; line2?: string; postal_code?: string; state?: string; }`\n  The customer's billing address\n  - `city?: string`\n    City name\n  - `country?: string`\n    Two-letter ISO country code\n  - `line1?: string`\n    Street address line 1\n  - `line2?: string`\n    Street address line 2 (apartment, suite, etc.)\n  - `postal_code?: string`\n    Postal or ZIP code\n  - `state?: string`\n    State, province, or region\n\n- `email?: string`\n  The customer's email address for receipts and notifications\n\n- `name?: string`\n  The customer's full name\n\n- `phone?: string`\n  The customer's phone number in E.164 format (e.g., +14155551234)\n\n### Returns\n\n- `{ id: string; created: string; environment: 'sandbox' | 'live'; address?: { city?: string; country?: string; line1?: string; line2?: string; postal_code?: string; state?: string; }; email?: string; name?: string; phone?: string; }`\n  A billable customer. Represents the entity in your app that will be charged—typically a user, organization, or project.\n\n  - `id: string`\n  - `created: string`\n  - `environment: 'sandbox' | 'live'`\n  - `address?: { city?: string; country?: string; line1?: string; line2?: string; postal_code?: string; state?: string; }`\n  - `email?: string`\n  - `name?: string`\n  - `phone?: string`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst customer = await client.commerce.customers.create();\n\nconsole.log(customer);\n```",
    perLanguage: {
      typescript: {
        method: 'client.commerce.customers.create',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst customer = await client.commerce.customers.create();\n\nconsole.log(customer.id);",
      },
      http: {
  ...
  },
  {
    ...
    response:
      "{ id: string; created: string; environment: 'sandbox' | 'live'; address?: { city?: string; country?: string; line1?: string; line2?: string; postal_code?: string; state?: string; }; email?: string; name?: string; phone?: string; }",
    markdown:
      "## update\n\n`client.commerce.customers.update(customer_id: string, address?: { city?: string; country?: string; line1?: string; line2?: string; postal_code?: string; state?: string; }, email?: string, name?: string, phone?: string): { id: string; created: string; environment: 'sandbox' | 'live'; address?: customer_address; email?: string; name?: string; phone?: string; }`\n\n**patch** `/v1/commerce/customers/{customer_id}`\n\nUpdates an existing customer. Use this to modify contact information or billing address. Only provided fields are updated; omitted fields remain unchanged.\n\n### Parameters\n\n- `customer_id: string`\n\n- `address?: { city?: string; country?: string; line1?: string; line2?: string; postal_code?: string; state?: string; }`\n  The customer's billing address\n  - `city?: string`\n    City name\n  - `country?: string`\n    Two-letter ISO country code\n  - `line1?: string`\n    Street address line 1\n  - `line2?: string`\n    Street address line 2 (apartment, suite, etc.)\n  - `postal_code?: string`\n    Postal or ZIP code\n  - `state?: string`\n    State, province, or region\n\n- `email?: string`\n  The customer's email address for receipts and notifications\n\n- `name?: string`\n  The customer's full name\n\n- `phone?: string`\n  The customer's phone number in E.164 format (e.g., +14155551234)\n\n### Returns\n\n- `{ id: string; created: string; environment: 'sandbox' | 'live'; address?: { city?: string; country?: string; line1?: string; line2?: string; postal_code?: string; state?: string; }; email?: string; name?: string; phone?: string; }`\n  A billable customer. Represents the entity in your app that will be charged—typically a user, organization, or project.\n\n  - `id: string`\n  - `created: string`\n  - `environment: 'sandbox' | 'live'`\n  - `address?: { city?: string; country?: string; line1?: string; line2?: string; postal_code?: string; state?: string; }`\n  - `email?: string`\n  - `name?: string`\n  - `phone?: string`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst customer = await client.commerce.customers.update('cus_1234567890');\n\nconsole.log(customer);\n```",
    perLanguage: {
      typescript: {
        method: 'client.commerce.customers.update',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst customer = await client.commerce.customers.update('cus_1234567890');\n\nconsole.log(customer.id);",
      },
      http: {
  ...
  },
  {
    ...
    response: "{ environment: 'sandbox' | 'live'; url: string; }",
    markdown:
      "## billing_portal\n\n`client.commerce.customers.billingPortal(customer_id: string, return_url?: string): { environment: 'sandbox' | 'live'; url: string; }`\n\n**post** `/v1/commerce/customers/{customer_id}/billing_portal`\n\nGenerates a URL to a hosted billing portal where the customer can view invoices, update payment methods, and manage billing details. Redirect the customer to the returned URL.\n\n### Parameters\n\n- `customer_id: string`\n\n- `return_url?: string`\n  URL to redirect the customer to after they exit the billing portal\n\n### Returns\n\n- `{ environment: 'sandbox' | 'live'; url: string; }`\n  Response containing the billing portal URL\n\n  - `environment: 'sandbox' | 'live'`\n  - `url: string`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst response = await client.commerce.customers.billingPortal('cus_1234567890');\n\nconsole.log(response);\n```",
    perLanguage: {
      typescript: {
        method: 'client.commerce.customers.billingPortal',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst response = await client.commerce.customers.billingPortal('cus_1234567890');\n\nconsole.log(response.environment);",
      },
      http: {
        example:
          "curl https://api.hercules.app/v1/commerce/customers/$CUSTOMER_ID/billing_portal \\\n    -H 'Content-Type: application/json' \\\n    -H \"Authorization: Bearer $HERCULES_API_KEY\" \\\n    -d '{}'",
      },
    },
  },
  {
    ...
    response:
      "{ id: string; active: boolean; created: string; environment: 'sandbox' | 'live'; name: string; subscription_group_id: string; description?: string; media?: { id: string; display_order: number; type: 'image' | 'video'; url: string; file_size?: number; filename?: string; thumbnail_url?: string; }[]; metadata?: object; resources?: { id: string; active: boolean; created: string; environment: 'sandbox' | 'live'; type: 'feature'; feature?: { id: string; metadata?: object; }; }[]; tags?: string[]; }",
    markdown:
      "## list\n\n`client.commerce.products.list(active?: boolean, customer_id?: string, ending_before?: string, limit?: number, starting_after?: string): { id: string; active: boolean; created: string; environment: 'sandbox' | 'live'; name: string; subscription_group_id: string; description?: string; media?: object[]; metadata?: object; resources?: object[]; tags?: string[]; }`\n\n**get** `/v1/commerce/products`\n\nRetrieves a paginated list of subscription products. Products define the pricing and billing intervals for subscriptions. Each product can have resources attached that grant access to features or content.\n\n### Parameters\n\n- `active?: boolean`\n  Filter by active status\n\n- `customer_id?: string`\n  The customer this catalog is being read for. Used only to resolve their environment when they hold a test-mode grant; it never filters the results.\n\n- `ending_before?: string`\n  Cursor for backward pagination. Pass the ID of the first product from the previous page.\n\n- `limit?: number`\n  Maximum number of products to return (1-100)\n\n- `starting_after?: string`\n  Cursor for forward pagination. Pass the ID of the last product from the previous page.\n\n### Returns\n\n- `{ id: string; active: boolean; created: string; environment: 'sandbox' | 'live'; name: string; subscription_group_id: string; description?: string; media?: { id: string; display_order: number; type: 'image' | 'video'; url: string; file_size?: number; filename?: string; thumbnail_url?: string; }[]; metadata?: object; resources?: { id: string; active: boolean; created: string; environment: 'sandbox' | 'live'; type: 'feature'; feature?: { id: string; metadata?: object; }; }[]; tags?: string[]; }`\n  A product that customers can purchase. Products can be one-time purchases or recurring subscriptions. Attach resources to a product to grant features to customers.\n\n  - `id: string`\n  - `active: boolean`\n  - `created: string`\n  - `environment: 'sandbox' | 'live'`\n  - `name: string`\n  - `subscription_group_id: string`\n  - `description?: string`\n  - `media?: { id: string; display_order: number; type: 'image' | 'video'; url: string; file_size?: number; filename?: string; thumbnail_url?: string; }[]`\n  - `metadata?: object`\n  - `resources?: { id: string; active: boolean; created: string; environment: 'sandbox' | 'live'; type: 'feature'; feature?: { id: string; metadata?: object; }; }[]`\n  - `tags?: string[]`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\n// Automatically fetches more pages as needed.\nfor await (const product of client.commerce.products.list()) {\n  console.log(product);\n}\n```",
    perLanguage: {
      typescript: {
        method: 'client.commerce.products.list',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\n// Automatically fetches more pages as needed.\nfor await (const product of client.commerce.products.list()) {\n  console.log(product.id);\n}",
      },
      http: {
        example:
          'curl https://api.hercules.app/v1/commerce/products \\\n    -H "Authorization: Bearer $HERCULES_API_KEY"',
      },
    },
  },
  {
    ...
    params: ['product_id: string;', 'customer_id?: string;'],
    response:
      "{ id: string; active: boolean; created: string; environment: 'sandbox' | 'live'; name: string; subscription_group_id: string; description?: string; media?: { id: string; display_order: number; type: 'image' | 'video'; url: string; file_size?: number; filename?: string; thumbnail_url?: string; }[]; metadata?: object; resources?: { id: string; active: boolean; created: string; environment: 'sandbox' | 'live'; type: 'feature'; feature?: { id: string; metadata?: object; }; }[]; tags?: string[]; }",
    markdown:
      "## get\n\n`client.commerce.products.get(product_id: string, customer_id?: string): { id: string; active: boolean; created: string; environment: 'sandbox' | 'live'; name: string; subscription_group_id: string; description?: string; media?: object[]; metadata?: object; resources?: object[]; tags?: string[]; }`\n\n**get** `/v1/commerce/products/{product_id}`\n\nRetrieves a product by ID. Returns the product object including pricing details and status.\n\n### Parameters\n\n- `product_id: string`\n\n- `customer_id?: string`\n  The customer this catalog is being read for. Used only to resolve their environment when they hold a test-mode grant; it never filters the results.\n\n### Returns\n\n- `{ id: string; active: boolean; created: string; environment: 'sandbox' | 'live'; name: string; subscription_group_id: string; description?: string; media?: { id: string; display_order: number; type: 'image' | 'video'; url: string; file_size?: number; filename?: string; thumbnail_url?: string; }[]; metadata?: object; resources?: { id: string; active: boolean; created: string; environment: 'sandbox' | 'live'; type: 'feature'; feature?: { id: string; metadata?: object; }; }[]; tags?: string[]; }`\n  A product that customers can purchase. Products can be one-time purchases or recurring subscriptions. Attach resources to a product to grant features to customers.\n\n  - `id: string`\n  - `active: boolean`\n  - `created: string`\n  - `environment: 'sandbox' | 'live'`\n  - `name: string`\n  - `subscription_group_id: string`\n  - `description?: string`\n  - `media?: { id: string; display_order: number; type: 'image' | 'video'; url: string; file_size?: number; filename?: string; thumbnail_url?: string; }[]`\n  - `metadata?: object`\n  - `resources?: { id: string; active: boolean; created: string; environment: 'sandbox' | 'live'; type: 'feature'; feature?: { id: string; metadata?: object; }; }[]`\n  - `tags?: string[]`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst product = await client.commerce.products.get('product_id');\n\nconsole.log(product);\n```",
    perLanguage: {
      typescript: {
        method: 'client.commerce.products.get',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst product = await client.commerce.products.get('product_id');\n\nconsole.log(product.id);",
      },
      http: {
        example:
          'curl https://api.hercules.app/v1/commerce/products/$PRODUCT_ID \\\n    -H "Authorization: Bearer $HERCULES_API_KEY"',
      },
    },
  },
  {
  ...
  },
  {
    ...
    response:
      "{ data: { id: string; active: boolean; created: string; environment: 'sandbox' | 'live'; type: 'feature'; feature?: { id: string; metadata?: object; }; }[]; environment: 'sandbox' | 'live'; }",
    markdown:
      "## attach\n\n`client.commerce.products.resources.attach(product_id: string, resource_ids: string[]): { data: object[]; environment: 'sandbox' | 'live'; }`\n\n**post** `/v1/commerce/products/{product_id}/resources`\n\nAttaches one or more resources to a product. Customers who purchase this product will gain access to the attached resources. Resource type is inferred from the ID prefix (e.g., 'feat_' for features).\n\n### Parameters\n\n- `product_id: string`\n\n- `resource_ids: string[]`\n  Resource IDs to attach. Type is inferred from ID prefix (e.g., 'feat_' for features).\n\n### Returns\n\n- `{ data: { id: string; active: boolean; created: string; environment: 'sandbox' | 'live'; type: 'feature'; feature?: { id: string; metadata?: object; }; }[]; environment: 'sandbox' | 'live'; }`\n  Response containing attached resources\n\n  - `data: { id: string; active: boolean; created: string; environment: 'sandbox' | 'live'; type: 'feature'; feature?: { id: string; metadata?: object; }; }[]`\n  - `environment: 'sandbox' | 'live'`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst response = await client.commerce.products.resources.attach('product_id', { resource_ids: ['string'] });\n\nconsole.log(response);\n```",
    perLanguage: {
      typescript: {
        method: 'client.commerce.products.resources.attach',
        example:
  ...
  },
  {
    ...
    perLanguage: {
      typescript: {
        method: 'client.commerce.products.resources.detach',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nawait client.commerce.products.resources.detach('product_id', { resource_ids: ['string'] });",
      },
      http: {
  ...
  },
  {
    ...
    response:
      "{ id: string; active: boolean; created: string; environment: 'sandbox' | 'live'; name: string; currency?: string; description?: string; media?: { id: string; display_order: number; type: 'image' | 'video'; url: string; file_size?: number; filename?: string; thumbnail_url?: string; }[]; metadata?: object; recurring?: { interval: 'day' | 'week' | 'month' | 'year'; interval_count?: number; }; unit_amount?: number; }",
    markdown:
      "## list\n\n`client.commerce.products.variants.list(product_id: string, active?: boolean, customer_id?: string, ending_before?: string, limit?: number, starting_after?: string): { id: string; active: boolean; created: string; environment: 'sandbox' | 'live'; name: string; currency?: currency; description?: string; media?: object[]; metadata?: object; recurring?: object; unit_amount?: number; }`\n\n**get** `/v1/commerce/products/{product_id}/variants`\n\nRetrieves all variants for a product. Variants represent different pricing tiers or configurations (e.g., Basic, Pro, Enterprise) within a single product.\n\n### Parameters\n\n- `product_id: string`\n\n- `active?: boolean`\n  Filter by active status\n\n- `customer_id?: string`\n  The customer this catalog is being read for. Used only to resolve their environment when they hold a test-mode grant; it never filters the results.\n\n- `ending_before?: string`\n  Cursor for backward pagination. Pass the ID of the first variant from the previous page.\n\n- `limit?: number`\n  Maximum number of variants to return (1-100)\n\n- `starting_after?: string`\n  Cursor for forward pagination. Pass the ID of the last variant from the previous page.\n\n### Returns\n\n- `{ id: string; active: boolean; created: string; environment: 'sandbox' | 'live'; name: string; currency?: string; description?: string; media?: { id: string; display_order: number; type: 'image' | 'video'; url: string; file_size?: number; filename?: string; thumbnail_url?: string; }[]; metadata?: object; recurring?: { interval: 'day' | 'week' | 'month' | 'year'; interval_count?: number; }; unit_amount?: number; }`\n  A product variant representing a specific tier or configuration. Each variant has its own pricing. Variants allow a single product to have multiple pricing options (e.g., Basic, Pro, Enterprise tiers).\n\n  - `id: string`\n  - `active: boolean`\n  - `created: string`\n  - `environment: 'sandbox' | 'live'`\n  - `name: string`\n  - `currency?: string`\n  - `description?: string`\n  - `media?: { id: string; display_order: number; type: 'image' | 'video'; url: string; file_size?: number; filename?: string; thumbnail_url?: string; }[]`\n  - `metadata?: object`\n  - `recurring?: { interval: 'day' | 'week' | 'month' | 'year'; interval_count?: number; }`\n  - `unit_amount?: number`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\n// Automatically fetches more pages as needed.\nfor await (const variant of client.commerce.products.variants.list('product_id')) {\n  console.log(variant);\n}\n```",
    perLanguage: {
      typescript: {
        method: 'client.commerce.products.variants.list',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\n// Automatically fetches more pages as needed.\nfor await (const variant of client.commerce.products.variants.list('product_id')) {\n  console.log(variant.id);\n}",
      },
      http: {
        example:
          'curl https://api.hercules.app/v1/commerce/products/$PRODUCT_ID/variants \\\n    -H "Authorization: Bearer $HERCULES_API_KEY"',
      },
    },
  },
  {
    ...
    params: ['product_id: string;', 'variant_id: string;', 'customer_id?: string;'],
    response:
      "{ id: string; active: boolean; created: string; environment: 'sandbox' | 'live'; name: string; currency?: string; description?: string; media?: { id: string; display_order: number; type: 'image' | 'video'; url: string; file_size?: number; filename?: string; thumbnail_url?: string; }[]; metadata?: object; recurring?: { interval: 'day' | 'week' | 'month' | 'year'; interval_count?: number; }; unit_amount?: number; }",
    markdown:
      "## get\n\n`client.commerce.products.variants.get(product_id: string, variant_id: string, customer_id?: string): { id: string; active: boolean; created: string; environment: 'sandbox' | 'live'; name: string; currency?: currency; description?: string; media?: object[]; metadata?: object; recurring?: object; unit_amount?: number; }`\n\n**get** `/v1/commerce/products/{product_id}/variants/{variant_id}`\n\nRetrieves a variant by ID. Returns the variant object including pricing details and status.\n\n### Parameters\n\n- `product_id: string`\n\n- `variant_id: string`\n\n- `customer_id?: string`\n  The customer this catalog is being read for. Used only to resolve their environment when they hold a test-mode grant; it never filters the results.\n\n### Returns\n\n- `{ id: string; active: boolean; created: string; environment: 'sandbox' | 'live'; name: string; currency?: string; description?: string; media?: { id: string; display_order: number; type: 'image' | 'video'; url: string; file_size?: number; filename?: string; thumbnail_url?: string; }[]; metadata?: object; recurring?: { interval: 'day' | 'week' | 'month' | 'year'; interval_count?: number; }; unit_amount?: number; }`\n  A product variant representing a specific tier or configuration. Each variant has its own pricing. Variants allow a single product to have multiple pricing options (e.g., Basic, Pro, Enterprise tiers).\n\n  - `id: string`\n  - `active: boolean`\n  - `created: string`\n  - `environment: 'sandbox' | 'live'`\n  - `name: string`\n  - `currency?: string`\n  - `description?: string`\n  - `media?: { id: string; display_order: number; type: 'image' | 'video'; url: string; file_size?: number; filename?: string; thumbnail_url?: string; }[]`\n  - `metadata?: object`\n  - `recurring?: { interval: 'day' | 'week' | 'month' | 'year'; interval_count?: number; }`\n  - `unit_amount?: number`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst variant = await client.commerce.products.variants.get('variant_id', { product_id: 'product_id' });\n\nconsole.log(variant);\n```",
    perLanguage: {
      typescript: {
        method: 'client.commerce.products.variants.get',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst variant = await client.commerce.products.variants.get('variant_id', {\n  product_id: 'product_id',\n});\n\nconsole.log(variant.id);",
      },
      http: {
        example:
          'curl https://api.hercules.app/v1/commerce/products/$PRODUCT_ID/variants/$VARIANT_ID \\\n    -H "Authorization: Bearer $HERCULES_API_KEY"',
      },
    },
  },
  {
  ...
  },
  {
    ...
    response:
      "{ id: string; active: boolean; created: string; environment: 'sandbox' | 'live'; name: string; currency?: string; description?: string; media?: { id: string; display_order: number; type: 'image' | 'video'; url: string; file_size?: number; filename?: string; thumbnail_url?: string; }[]; metadata?: object; recurring?: { interval: 'day' | 'week' | 'month' | 'year'; interval_count?: number; }; unit_amount?: number; }",
    markdown:
      "## update\n\n`client.commerce.products.variants.update(product_id: string, variant_id: string, active?: boolean, description?: string, media?: { cdn_file_id: string; type: 'image' | 'video'; display_order?: number; }[], metadata?: object, name?: string): { id: string; active: boolean; created: string; environment: 'sandbox' | 'live'; name: string; currency?: currency; description?: string; media?: object[]; metadata?: object; recurring?: object; unit_amount?: number; }`\n\n**patch** `/v1/commerce/products/{product_id}/variants/{variant_id}`\n\nUpdates an existing variant. Use this to modify the name, description, or active status. Pricing cannot be changed after creation—create a new variant instead.\n\n### Parameters\n\n- `product_id: string`\n\n- `variant_id: string`\n\n- `active?: boolean`\n  Whether the variant is available for new purchases\n\n- `description?: string`\n  Detailed description of what this variant includes\n\n- `media?: { cdn_file_id: string; type: 'image' | 'video'; display_order?: number; }[]`\n  Replace all media attachments. Pass empty array to remove all media.\n\n- `metadata?: object`\n  Custom metadata for the variant\n\n- `name?: string`\n  Display name for the variant\n\n### Returns\n\n- `{ id: string; active: boolean; created: string; environment: 'sandbox' | 'live'; name: string; currency?: string; description?: string; media?: { id: string; display_order: number; type: 'image' | 'video'; url: string; file_size?: number; filename?: string; thumbnail_url?: string; }[]; metadata?: object; recurring?: { interval: 'day' | 'week' | 'month' | 'year'; interval_count?: number; }; unit_amount?: number; }`\n  A product variant representing a specific tier or configuration. Each variant has its own pricing. Variants allow a single product to have multiple pricing options (e.g., Basic, Pro, Enterprise tiers).\n\n  - `id: string`\n  - `active: boolean`\n  - `created: string`\n  - `environment: 'sandbox' | 'live'`\n  - `name: string`\n  - `currency?: string`\n  - `description?: string`\n  - `media?: { id: string; display_order: number; type: 'image' | 'video'; url: string; file_size?: number; filename?: string; thumbnail_url?: string; }[]`\n  - `metadata?: object`\n  - `recurring?: { interval: 'day' | 'week' | 'month' | 'year'; interval_count?: number; }`\n  - `unit_amount?: number`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst variant = await client.commerce.products.variants.update('variant_id', { product_id: 'product_id' });\n\nconsole.log(variant);\n```",
    perLanguage: {
      typescript: {
        method: 'client.commerce.products.variants.update',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst variant = await client.commerce.products.variants.update('variant_id', {\n  product_id: 'product_id',\n});\n\nconsole.log(variant.id);",
      },
      http: {
        example:
          "curl https://api.hercules.app/v1/commerce/products/$PRODUCT_ID/variants/$VARIANT_ID \\\n    -X PATCH \\\n    -H 'Content-Type: application/json' \\\n    -H \"Authorization: Bearer $HERCULES_API_KEY\" \\\n    -d '{}'",
  ...
  },
  {
    ...
    response:
      "{ id: string; active: boolean; code: string; created: string; duration: 'once' | 'repeating' | 'forever'; environment: 'sandbox' | 'live'; times_redeemed: number; amount_off?: number; currency?: string; duration_in_months?: number; max_redemptions?: number; name?: string; percent_off?: number; redeem_by?: string; }",
    markdown:
      "## list\n\n`client.commerce.coupons.list(active?: boolean, ending_before?: string, limit?: number, starting_after?: string): { id: string; active: boolean; code: string; created: string; duration: 'once' | 'repeating' | 'forever'; environment: 'sandbox' | 'live'; times_redeemed: number; amount_off?: number; currency?: string; duration_in_months?: number; max_redemptions?: number; name?: string; percent_off?: number; redeem_by?: string; }`\n\n**get** `/v1/commerce/coupons`\n\nRetrieves a paginated list of all coupons. Coupons provide discounts that customers can apply during checkout using a promo code.\n\n### Parameters\n\n- `active?: boolean`\n  Filter by active status\n\n- `ending_before?: string`\n  Cursor for backward pagination. Pass the ID of the first coupon from the previous page.\n\n- `limit?: number`\n  Maximum number of coupons to return (1-100)\n\n- `starting_after?: string`\n  Cursor for forward pagination. Pass the ID of the last coupon from the previous page.\n\n### Returns\n\n- `{ id: string; active: boolean; code: string; created: string; duration: 'once' | 'repeating' | 'forever'; environment: 'sandbox' | 'live'; times_redeemed: number; amount_off?: number; currency?: string; duration_in_months?: number; max_redemptions?: number; name?: string; percent_off?: number; redeem_by?: string; }`\n  A discount coupon that customers can apply during checkout using a promo code\n\n  - `id: string`\n  - `active: boolean`\n  - `code: string`\n  - `created: string`\n  - `duration: 'once' | 'repeating' | 'forever'`\n  - `environment: 'sandbox' | 'live'`\n  - `times_redeemed: number`\n  - `amount_off?: number`\n  - `currency?: string`\n  - `duration_in_months?: number`\n  - `max_redemptions?: number`\n  - `name?: string`\n  - `percent_off?: number`\n  - `redeem_by?: string`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\n// Automatically fetches more pages as needed.\nfor await (const coupon of client.commerce.coupons.list()) {\n  console.log(coupon);\n}\n```",
    perLanguage: {
      typescript: {
        method: 'client.commerce.coupons.list',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\n// Automatically fetches more pages as needed.\nfor await (const coupon of client.commerce.coupons.list()) {\n  console.log(coupon.id);\n}",
      },
      http: {
        example:
          'curl https://api.hercules.app/v1/commerce/coupons \\\n    -H "Authorization: Bearer $HERCULES_API_KEY"',
      },
    },
  },
  {
    ...
    response:
      "{ id: string; active: boolean; code: string; created: string; duration: 'once' | 'repeating' | 'forever'; environment: 'sandbox' | 'live'; times_redeemed: number; amount_off?: number; currency?: string; duration_in_months?: number; max_redemptions?: number; name?: string; percent_off?: number; redeem_by?: string; }",
    markdown:
      "## get\n\n`client.commerce.coupons.get(coupon_id: string): { id: string; active: boolean; code: string; created: string; duration: 'once' | 'repeating' | 'forever'; environment: 'sandbox' | 'live'; times_redeemed: number; amount_off?: number; currency?: string; duration_in_months?: number; max_redemptions?: number; name?: string; percent_off?: number; redeem_by?: string; }`\n\n**get** `/v1/commerce/coupons/{coupon_id}`\n\nRetrieves a coupon by ID. Returns the coupon object including discount details and redemption statistics.\n\n### Parameters\n\n- `coupon_id: string`\n\n### Returns\n\n- `{ id: string; active: boolean; code: string; created: string; duration: 'once' | 'repeating' | 'forever'; environment: 'sandbox' | 'live'; times_redeemed: number; amount_off?: number; currency?: string; duration_in_months?: number; max_redemptions?: number; name?: string; percent_off?: number; redeem_by?: string; }`\n  A discount coupon that customers can apply during checkout using a promo code\n\n  - `id: string`\n  - `active: boolean`\n  - `code: string`\n  - `created: string`\n  - `duration: 'once' | 'repeating' | 'forever'`\n  - `environment: 'sandbox' | 'live'`\n  - `times_redeemed: number`\n  - `amount_off?: number`\n  - `currency?: string`\n  - `duration_in_months?: number`\n  - `max_redemptions?: number`\n  - `name?: string`\n  - `percent_off?: number`\n  - `redeem_by?: string`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst coupon = await client.commerce.coupons.get('coupon_1234567890');\n\nconsole.log(coupon);\n```",
    perLanguage: {
      typescript: {
        method: 'client.commerce.coupons.get',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst coupon = await client.commerce.coupons.get('coupon_1234567890');\n\nconsole.log(coupon.id);",
      },
      http: {
        example:
          'curl https://api.hercules.app/v1/commerce/coupons/$COUPON_ID \\\n    -H "Authorization: Bearer $HERCULES_API_KEY"',
      },
    },
  },
  {
  ...
  },
  {
    ...
    response:
      "{ id: string; active: boolean; code: string; created: string; duration: 'once' | 'repeating' | 'forever'; environment: 'sandbox' | 'live'; times_redeemed: number; amount_off?: number; currency?: string; duration_in_months?: number; max_redemptions?: number; name?: string; percent_off?: number; redeem_by?: string; }",
    markdown:
      "## update\n\n`client.commerce.coupons.update(coupon_id: string, active?: boolean, name?: string): { id: string; active: boolean; code: string; created: string; duration: 'once' | 'repeating' | 'forever'; environment: 'sandbox' | 'live'; times_redeemed: number; amount_off?: number; currency?: string; duration_in_months?: number; max_redemptions?: number; name?: string; percent_off?: number; redeem_by?: string; }`\n\n**patch** `/v1/commerce/coupons/{coupon_id}`\n\nUpdates an existing coupon. Use this to modify the display name or deactivate the coupon. Discount amounts and codes cannot be changed after creation.\n\n### Parameters\n\n- `coupon_id: string`\n\n- `active?: boolean`\n  Whether the coupon is active and can be redeemed\n\n- `name?: string`\n  Display name for the coupon\n\n### Returns\n\n- `{ id: string; active: boolean; code: string; created: string; duration: 'once' | 'repeating' | 'forever'; environment: 'sandbox' | 'live'; times_redeemed: number; amount_off?: number; currency?: string; duration_in_months?: number; max_redemptions?: number; name?: string; percent_off?: number; redeem_by?: string; }`\n  A discount coupon that customers can apply during checkout using a promo code\n\n  - `id: string`\n  - `active: boolean`\n  - `code: string`\n  - `created: string`\n  - `duration: 'once' | 'repeating' | 'forever'`\n  - `environment: 'sandbox' | 'live'`\n  - `times_redeemed: number`\n  - `amount_off?: number`\n  - `currency?: string`\n  - `duration_in_months?: number`\n  - `max_redemptions?: number`\n  - `name?: string`\n  - `percent_off?: number`\n  - `redeem_by?: string`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst coupon = await client.commerce.coupons.update('coupon_1234567890');\n\nconsole.log(coupon);\n```",
    perLanguage: {
      typescript: {
        method: 'client.commerce.coupons.update',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst coupon = await client.commerce.coupons.update('coupon_1234567890');\n\nconsole.log(coupon.id);",
      },
      http: {
  ...
  },
  {
    ...
    response:
      "{ id: string; active: boolean; created: string; description: string; environment: 'sandbox' | 'live'; name: string; metadata?: object; }",
    markdown:
      "## get\n\n`client.commerce.features.get(feature_id: string): { id: string; active: boolean; created: string; description: string; environment: 'sandbox' | 'live'; name: string; metadata?: object; }`\n\n**get** `/v1/commerce/features/{feature_id}`\n\nRetrieves a feature by ID.\n\n### Parameters\n\n- `feature_id: string`\n\n### Returns\n\n- `{ id: string; active: boolean; created: string; description: string; environment: 'sandbox' | 'live'; name: string; metadata?: object; }`\n  A feature that can be attached to multiple products. Features represent monetizable content or functionality in your app.\n\n  - `id: string`\n  - `active: boolean`\n  - `created: string`\n  - `description: string`\n  - `environment: 'sandbox' | 'live'`\n  - `name: string`\n  - `metadata?: object`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst feature = await client.commerce.features.get('feature_id');\n\nconsole.log(feature);\n```",
    perLanguage: {
      typescript: {
        method: 'client.commerce.features.get',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst feature = await client.commerce.features.get('feature_id');\n\nconsole.log(feature.id);",
      },
      http: {
        example:
          'curl https://api.hercules.app/v1/commerce/features/$FEATURE_ID \\\n    -H "Authorization: Bearer $HERCULES_API_KEY"',
      },
    },
  },
  {
  ...
  },
  {
    ...
    response:
      "{ id: string; active: boolean; created: string; description: string; environment: 'sandbox' | 'live'; name: string; metadata?: object; }",
    markdown:
      "## update\n\n`client.commerce.features.update(feature_id: string, active?: boolean, description?: string, metadata?: object, name?: string): { id: string; active: boolean; created: string; description: string; environment: 'sandbox' | 'live'; name: string; metadata?: object; }`\n\n**patch** `/v1/commerce/features/{feature_id}`\n\nUpdates an existing feature. Use this to modify metadata or active status.\n\n### Parameters\n\n- `feature_id: string`\n\n- `active?: boolean`\n  Whether the feature is active\n\n- `description?: string`\n  Description of what this feature provides\n\n- `metadata?: object`\n  Custom metadata for the feature\n\n- `name?: string`\n  Human-readable name for the feature\n\n### Returns\n\n- `{ id: string; active: boolean; created: string; description: string; environment: 'sandbox' | 'live'; name: string; metadata?: object; }`\n  A feature that can be attached to multiple products. Features represent monetizable content or functionality in your app.\n\n  - `id: string`\n  - `active: boolean`\n  - `created: string`\n  - `description: string`\n  - `environment: 'sandbox' | 'live'`\n  - `name: string`\n  - `metadata?: object`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst feature = await client.commerce.features.update('feature_id');\n\nconsole.log(feature);\n```",
    perLanguage: {
      typescript: {
        method: 'client.commerce.features.update',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst feature = await client.commerce.features.update('feature_id');\n\nconsole.log(feature.id);",
      },
      http: {
  ...
  },
  {
    ...
    perLanguage: {
      typescript: {
        method: 'client.content.collections.list',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\n// Automatically fetches more pages as needed.\nfor await (const collection of client.content.collections.list()) {\n  console.log(collection.id);\n}",
      },
      http: {
        example:
          'curl https://api.hercules.app/v1/content/collections \\\n    -H "Authorization: Bearer $HERCULES_API_KEY"',
      },
    },
  },
  {
    ...
    perLanguage: {
      typescript: {
        method: 'client.content.collections.get',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst collection = await client.content.collections.get('collection_id');\n\nconsole.log(collection.id);",
      },
      http: {
        example:
          'curl https://api.hercules.app/v1/content/collections/$COLLECTION_ID \\\n    -H "Authorization: Bearer $HERCULES_API_KEY"',
      },
    },
  },
  {
    ...
    perLanguage: {
      typescript: {
        method: 'client.content.collections.create',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst collection = await client.content.collections.create({ api_id: 'nXI', name: 'name' });\n\nconsole.log(collection.id);",
      },
      http: {
  ...
  },
  {
    ...
    perLanguage: {
      typescript: {
        method: 'client.content.collections.update',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst collection = await client.content.collections.update('collection_id');\n\nconsole.log(collection.id);",
      },
      http: {
  ...
  },
  {
    ...
    perLanguage: {
      typescript: {
        method: 'client.content.collections.archive',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst collection = await client.content.collections.archive('collection_id');\n\nconsole.log(collection.id);",
      },
      http: {
  ...
  },
  {
    ...
    perLanguage: {
      typescript: {
        method: 'client.content.collections.fields.create',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst field = await client.content.collections.fields.create('collection_id', {\n  api_id: 'nXI',\n  name: 'name',\n  type: 'text',\n});\n\nconsole.log(field.id);",
      },
      http: {
        example:
          'curl https://api.hercules.app/v1/content/collections/$COLLECTION_ID/fields \\\n    -H \'Content-Type: application/json\' \\\n    -H "Authorization: Bearer $HERCULES_API_KEY" \\\n    -d \'{\n          "api_id": "nXI",\n          "name": "name",\n          "type": "text"\n        }\'',
      },
    },
  },
  {
    ...
    perLanguage: {
      typescript: {
        method: 'client.content.collections.fields.update',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst field = await client.content.collections.fields.update('field_id', {\n  collection_id: 'collection_id',\n});\n\nconsole.log(field.id);",
      },
      http: {
        example:
          "curl https://api.hercules.app/v1/content/collections/$COLLECTION_ID/fields/$FIELD_ID \\\n    -X PATCH \\\n    -H 'Content-Type: application/json' \\\n    -H \"Authorization: Bearer $HERCULES_API_KEY\" \\\n    -d '{}'",
      },
    },
  },
  {
    ...
    perLanguage: {
      typescript: {
        method: 'client.content.collections.fields.delete',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nawait client.content.collections.fields.delete('field_id', { collection_id: 'collection_id' });",
      },
      http: {
  ...
  },
  {
    ...
    response:
      "{ id: string; collection_id: string; created: string; fields: object; status: 'draft' | 'published' | 'archived'; updated: string; version: number; collection_api_id?: string; first_published_at?: string; published_at?: string; scheduled_publish_at?: string; scheduled_unpublish_at?: string; }",
    markdown:
      "## list\n\n`client.content.entries.list(collection?: string, created?: { gt?: number; gte?: number; lt?: number; lte?: number; }, ending_before?: string, include_depth?: number, limit?: number, order_by?: string, published?: { gt?: number; gte?: number; lt?: number; lte?: number; }, query?: string, starting_after?: string, status?: 'draft' | 'published' | 'archived', updated?: { gt?: number; gte?: number; lt?: number; lte?: number; }, where?: string): { id: string; collection_id: string; created: string; fields: object; status: 'draft' | 'published' | 'archived'; updated: string; version: number; collection_api_id?: string; first_published_at?: string; published_at?: string; scheduled_publish_at?: string; scheduled_unpublish_at?: string; }`\n\n**get** `/v1/content/entries`\n\nRetrieves a paginated list of content entries. Supports filtering by collection, status, locale, and custom field queries. Use the 'where' parameter for field-based filtering with operators like $eq, $contains, $gt, etc.\n\n### Parameters\n\n- `collection?: string`\n  Filter by content collection API ID (e.g., 'blogPost')\n\n- `created?: { gt?: number; gte?: number; lt?: number; lte?: number; }`\n  Filter by creation date\n  - `gt?: number`\n    Greater than (Unix timestamp in seconds)\n  - `gte?: number`\n    Greater than or equal (Unix timestamp in seconds)\n  - `lt?: number`\n    Less than (Unix timestamp in seconds)\n  - `lte?: number`\n    Less than or equal (Unix timestamp in seconds)\n\n- `ending_before?: string`\n  Cursor for backward pagination. Pass the ID of the first entry from the previous page.\n\n- `include_depth?: number`\n  Depth for populating reference fields (0-3)\n\n- `limit?: number`\n  Maximum number of entries to return (1-100)\n\n- `order_by?: string`\n  JSON-encoded sort order. Example: {\"field\":\"publishedAt\",\"direction\":\"desc\"}\n\n- `published?: { gt?: number; gte?: number; lt?: number; lte?: number; }`\n  Filter by publish date\n  - `gt?: number`\n    Greater than (Unix timestamp in seconds)\n  - `gte?: number`\n    Greater than or equal (Unix timestamp in seconds)\n  - `lt?: number`\n    Less than (Unix timestamp in seconds)\n  - `lte?: number`\n    Less than or equal (Unix timestamp in seconds)\n\n- `query?: string`\n  Full-text search across text fields\n\n- `starting_after?: string`\n  Cursor for forward pagination. Pass the ID of the last entry from the previous page.\n\n- `status?: 'draft' | 'published' | 'archived'`\n  Filter by status\n\n- `updated?: { gt?: number; gte?: number; lt?: number; lte?: number; }`\n  Filter by update date\n  - `gt?: number`\n    Greater than (Unix timestamp in seconds)\n  - `gte?: number`\n    Greater than or equal (Unix timestamp in seconds)\n  - `lt?: number`\n    Less than (Unix timestamp in seconds)\n  - `lte?: number`\n    Less than or equal (Unix timestamp in seconds)\n\n- `where?: string`\n  JSON-encoded field filters. Example: {\"title\":{\"$contains\":\"hello\"}}\n\n### Returns\n\n- `{ id: string; collection_id: string; created: string; fields: object; status: 'draft' | 'published' | 'archived'; updated: string; version: number; collection_api_id?: string; first_published_at?: string; published_at?: string; scheduled_publish_at?: string; scheduled_unpublish_at?: string; }`\n  A content entry containing field values based on its collection schema. Entries can be drafts, published, or archived.\n\n  - `id: string`\n  - `collection_id: string`\n  - `created: string`\n  - `fields: object`\n  - `status: 'draft' | 'published' | 'archived'`\n  - `updated: string`\n  - `version: number`\n  - `collection_api_id?: string`\n  - `first_published_at?: string`\n  - `published_at?: string`\n  - `scheduled_publish_at?: string`\n  - `scheduled_unpublish_at?: string`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\n// Automatically fetches more pages as needed.\nfor await (const entry of client.content.entries.list()) {\n  console.log(entry);\n}\n```",
    perLanguage: {
      typescript: {
        method: 'client.content.entries.list',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\n// Automatically fetches more pages as needed.\nfor await (const entry of client.content.entries.list()) {\n  console.log(entry.id);\n}",
      },
      http: {
        example:
          'curl https://api.hercules.app/v1/content/entries \\\n    -H "Authorization: Bearer $HERCULES_API_KEY"',
      },
    },
  },
  {
    ...
    response:
      "{ id: string; collection_id: string; created: string; fields: object; status: 'draft' | 'published' | 'archived'; updated: string; version: number; collection_api_id?: string; first_published_at?: string; published_at?: string; scheduled_publish_at?: string; scheduled_unpublish_at?: string; }",
    markdown:
      "## get\n\n`client.content.entries.get(entry_id: string): { id: string; collection_id: string; created: string; fields: object; status: 'draft' | 'published' | 'archived'; updated: string; version: number; collection_api_id?: string; first_published_at?: string; published_at?: string; scheduled_publish_at?: string; scheduled_unpublish_at?: string; }`\n\n**get** `/v1/content/entries/{entry_id}`\n\nRetrieves a content entry by ID. Optionally specify a locale to get localized field values with fallback resolution.\n\n### Parameters\n\n- `entry_id: string`\n\n### Returns\n\n- `{ id: string; collection_id: string; created: string; fields: object; status: 'draft' | 'published' | 'archived'; updated: string; version: number; collection_api_id?: string; first_published_at?: string; published_at?: string; scheduled_publish_at?: string; scheduled_unpublish_at?: string; }`\n  A content entry containing field values based on its collection schema. Entries can be drafts, published, or archived.\n\n  - `id: string`\n  - `collection_id: string`\n  - `created: string`\n  - `fields: object`\n  - `status: 'draft' | 'published' | 'archived'`\n  - `updated: string`\n  - `version: number`\n  - `collection_api_id?: string`\n  - `first_published_at?: string`\n  - `published_at?: string`\n  - `scheduled_publish_at?: string`\n  - `scheduled_unpublish_at?: string`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst entry = await client.content.entries.get('entry_id');\n\nconsole.log(entry);\n```",
    perLanguage: {
      typescript: {
        method: 'client.content.entries.get',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst entry = await client.content.entries.get('entry_id');\n\nconsole.log(entry.id);",
      },
      http: {
        example:
          'curl https://api.hercules.app/v1/content/entries/$ENTRY_ID \\\n    -H "Authorization: Bearer $HERCULES_API_KEY"',
      },
    },
  },
  {
    ...
    response:
      "{ id: string; collection_id: string; created: string; fields: object; status: 'draft' | 'published' | 'archived'; updated: string; version: number; collection_api_id?: string; first_published_at?: string; published_at?: string; scheduled_publish_at?: string; scheduled_unpublish_at?: string; }",
    markdown:
      "## create\n\n`client.content.entries.create(collection: string, id?: string, fields?: object, status?: 'draft' | 'published' | 'archived'): { id: string; collection_id: string; created: string; fields: object; status: 'draft' | 'published' | 'archived'; updated: string; version: number; collection_api_id?: string; first_published_at?: string; published_at?: string; scheduled_publish_at?: string; scheduled_unpublish_at?: string; }`\n\n**post** `/v1/content/entries`\n\nCreates a new content entry for a given collection. Entries start as drafts by default. Use the publish endpoint to make entries publicly accessible.\n\n### Parameters\n\n- `collection: string`\n  API ID of the content collection (e.g., 'blogPost')\n\n- `id?: string`\n  Optional custom ID for the entry. Must start with 'ce_'. If not provided, one will be generated.\n\n- `fields?: object`\n  Field values. Localization is field-level: localized fields use { fieldApiId: { locale: value } }, non-localized use { fieldApiId: value }.\n\n- `status?: 'draft' | 'published' | 'archived'`\n  Initial status (default: draft)\n\n### Returns\n\n- `{ id: string; collection_id: string; created: string; fields: object; status: 'draft' | 'published' | 'archived'; updated: string; version: number; collection_api_id?: string; first_published_at?: string; published_at?: string; scheduled_publish_at?: string; scheduled_unpublish_at?: string; }`\n  A content entry containing field values based on its collection schema. Entries can be drafts, published, or archived.\n\n  - `id: string`\n  - `collection_id: string`\n  - `created: string`\n  - `fields: object`\n  - `status: 'draft' | 'published' | 'archived'`\n  - `updated: string`\n  - `version: number`\n  - `collection_api_id?: string`\n  - `first_published_at?: string`\n  - `published_at?: string`\n  - `scheduled_publish_at?: string`\n  - `scheduled_unpublish_at?: string`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst entry = await client.content.entries.create({ collection: 'collection' });\n\nconsole.log(entry);\n```",
    perLanguage: {
      typescript: {
        method: 'client.content.entries.create',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst entry = await client.content.entries.create({ collection: 'collection' });\n\nconsole.log(entry.id);",
      },
      http: {
  ...
  },
  {
    ...
    params: ['entry_id: string;', 'fields?: object;', 'version?: number;'],
    response:
      "{ id: string; collection_id: string; created: string; fields: object; status: 'draft' | 'published' | 'archived'; updated: string; version: number; collection_api_id?: string; first_published_at?: string; published_at?: string; scheduled_publish_at?: string; scheduled_unpublish_at?: string; }",
    markdown:
      "## update\n\n`client.content.entries.update(entry_id: string, fields?: object, version?: number): { id: string; collection_id: string; created: string; fields: object; status: 'draft' | 'published' | 'archived'; updated: string; version: number; collection_api_id?: string; first_published_at?: string; published_at?: string; scheduled_publish_at?: string; scheduled_unpublish_at?: string; }`\n\n**patch** `/v1/content/entries/{entry_id}`\n\nUpdates an existing content entry. Supports partial updates - only specified fields are modified. Use the version parameter for optimistic locking to prevent concurrent update conflicts.\n\n### Parameters\n\n- `entry_id: string`\n\n- `fields?: object`\n  Field values to update. Supports partial updates - only specified fields are changed.\n\n- `version?: number`\n  Expected version for optimistic locking. Update fails if version mismatch.\n\n### Returns\n\n- `{ id: string; collection_id: string; created: string; fields: object; status: 'draft' | 'published' | 'archived'; updated: string; version: number; collection_api_id?: string; first_published_at?: string; published_at?: string; scheduled_publish_at?: string; scheduled_unpublish_at?: string; }`\n  A content entry containing field values based on its collection schema. Entries can be drafts, published, or archived.\n\n  - `id: string`\n  - `collection_id: string`\n  - `created: string`\n  - `fields: object`\n  - `status: 'draft' | 'published' | 'archived'`\n  - `updated: string`\n  - `version: number`\n  - `collection_api_id?: string`\n  - `first_published_at?: string`\n  - `published_at?: string`\n  - `scheduled_publish_at?: string`\n  - `scheduled_unpublish_at?: string`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst entry = await client.content.entries.update('entry_id');\n\nconsole.log(entry);\n```",
    perLanguage: {
      typescript: {
        method: 'client.content.entries.update',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst entry = await client.content.entries.update('entry_id');\n\nconsole.log(entry.id);",
      },
      http: {
  ...
  },
  {
    ...
    response:
      "{ id: string; collection_id: string; created: string; fields: object; status: 'draft' | 'published' | 'archived'; updated: string; version: number; collection_api_id?: string; first_published_at?: string; published_at?: string; scheduled_publish_at?: string; scheduled_unpublish_at?: string; }",
    markdown:
      "## archive\n\n`client.content.entries.archive(entry_id: string): { id: string; collection_id: string; created: string; fields: object; status: 'draft' | 'published' | 'archived'; updated: string; version: number; collection_api_id?: string; first_published_at?: string; published_at?: string; scheduled_publish_at?: string; scheduled_unpublish_at?: string; }`\n\n**delete** `/v1/content/entries/{entry_id}`\n\nArchives a content entry, hiding it from the API and public access. The entry data is preserved and can be restored later.\n\n### Parameters\n\n- `entry_id: string`\n\n### Returns\n\n- `{ id: string; collection_id: string; created: string; fields: object; status: 'draft' | 'published' | 'archived'; updated: string; version: number; collection_api_id?: string; first_published_at?: string; published_at?: string; scheduled_publish_at?: string; scheduled_unpublish_at?: string; }`\n  A content entry containing field values based on its collection schema. Entries can be drafts, published, or archived.\n\n  - `id: string`\n  - `collection_id: string`\n  - `created: string`\n  - `fields: object`\n  - `status: 'draft' | 'published' | 'archived'`\n  - `updated: string`\n  - `version: number`\n  - `collection_api_id?: string`\n  - `first_published_at?: string`\n  - `published_at?: string`\n  - `scheduled_publish_at?: string`\n  - `scheduled_unpublish_at?: string`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst entry = await client.content.entries.archive('entry_id');\n\nconsole.log(entry);\n```",
    perLanguage: {
      typescript: {
        method: 'client.content.entries.archive',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst entry = await client.content.entries.archive('entry_id');\n\nconsole.log(entry.id);",
      },
      http: {
  ...
  },
  {
    ...
    response:
      "{ id: string; collection_id: string; created: string; fields: object; status: 'draft' | 'published' | 'archived'; updated: string; version: number; collection_api_id?: string; first_published_at?: string; published_at?: string; scheduled_publish_at?: string; scheduled_unpublish_at?: string; }",
    markdown:
      "## publish\n\n`client.content.entries.publish(entry_id: string, scheduled_at?: string): { id: string; collection_id: string; created: string; fields: object; status: 'draft' | 'published' | 'archived'; updated: string; version: number; collection_api_id?: string; first_published_at?: string; published_at?: string; scheduled_publish_at?: string; scheduled_unpublish_at?: string; }`\n\n**post** `/v1/content/entries/{entry_id}/publish`\n\nPublishes a content entry, making it publicly accessible. Optionally schedule publishing for a future time. Publishing validates that all required fields have values.\n\n### Parameters\n\n- `entry_id: string`\n\n- `scheduled_at?: string`\n  Schedule publish for a future time instead of publishing immediately\n\n### Returns\n\n- `{ id: string; collection_id: string; created: string; fields: object; status: 'draft' | 'published' | 'archived'; updated: string; version: number; collection_api_id?: string; first_published_at?: string; published_at?: string; scheduled_publish_at?: string; scheduled_unpublish_at?: string; }`\n  A content entry containing field values based on its collection schema. Entries can be drafts, published, or archived.\n\n  - `id: string`\n  - `collection_id: string`\n  - `created: string`\n  - `fields: object`\n  - `status: 'draft' | 'published' | 'archived'`\n  - `updated: string`\n  - `version: number`\n  - `collection_api_id?: string`\n  - `first_published_at?: string`\n  - `published_at?: string`\n  - `scheduled_publish_at?: string`\n  - `scheduled_unpublish_at?: string`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst entry = await client.content.entries.publish('entry_id');\n\nconsole.log(entry);\n```",
    perLanguage: {
      typescript: {
        method: 'client.content.entries.publish',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst entry = await client.content.entries.publish('entry_id');\n\nconsole.log(entry.id);",
      },
      http: {
        example:
          "curl https://api.hercules.app/v1/content/entries/$ENTRY_ID/publish \\\n    -H 'Content-Type: application/json' \\\n    -H \"Authorization: Bearer $HERCULES_API_KEY\" \\\n    -d '{}'",
      },
    },
  },
  {
    ...
    response:
      "{ id: string; collection_id: string; created: string; fields: object; status: 'draft' | 'published' | 'archived'; updated: string; version: number; collection_api_id?: string; first_published_at?: string; published_at?: string; scheduled_publish_at?: string; scheduled_unpublish_at?: string; }",
    markdown:
      "## unpublish\n\n`client.content.entries.unpublish(entry_id: string): { id: string; collection_id: string; created: string; fields: object; status: 'draft' | 'published' | 'archived'; updated: string; version: number; collection_api_id?: string; first_published_at?: string; published_at?: string; scheduled_publish_at?: string; scheduled_unpublish_at?: string; }`\n\n**post** `/v1/content/entries/{entry_id}/unpublish`\n\nUnpublishes a content entry, reverting it to draft status. The entry will no longer be publicly accessible but all data is preserved.\n\n### Parameters\n\n- `entry_id: string`\n\n### Returns\n\n- `{ id: string; collection_id: string; created: string; fields: object; status: 'draft' | 'published' | 'archived'; updated: string; version: number; collection_api_id?: string; first_published_at?: string; published_at?: string; scheduled_publish_at?: string; scheduled_unpublish_at?: string; }`\n  A content entry containing field values based on its collection schema. Entries can be drafts, published, or archived.\n\n  - `id: string`\n  - `collection_id: string`\n  - `created: string`\n  - `fields: object`\n  - `status: 'draft' | 'published' | 'archived'`\n  - `updated: string`\n  - `version: number`\n  - `collection_api_id?: string`\n  - `first_published_at?: string`\n  - `published_at?: string`\n  - `scheduled_publish_at?: string`\n  - `scheduled_unpublish_at?: string`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst entry = await client.content.entries.unpublish('entry_id');\n\nconsole.log(entry);\n```",
    perLanguage: {
      typescript: {
        method: 'client.content.entries.unpublish',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst entry = await client.content.entries.unpublish('entry_id');\n\nconsole.log(entry.id);",
      },
      http: {
        example:
          'curl https://api.hercules.app/v1/content/entries/$ENTRY_ID/unpublish \\\n    -X POST \\\n    -H "Authorization: Bearer $HERCULES_API_KEY"',
      },
    },
  },
  {
    ...
    perLanguage: {
      typescript: {
        method: 'client.content.assets.list',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\n// Automatically fetches more pages as needed.\nfor await (const asset of client.content.assets.list()) {\n  console.log(asset.id);\n}",
      },
      http: {
        example:
          'curl https://api.hercules.app/v1/content/assets \\\n    -H "Authorization: Bearer $HERCULES_API_KEY"',
      },
    },
  },
  {
    ...
    perLanguage: {
      typescript: {
        method: 'client.content.assets.get',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst asset = await client.content.assets.get('asset_id');\n\nconsole.log(asset.id);",
      },
      http: {
        example:
          'curl https://api.hercules.app/v1/content/assets/$ASSET_ID \\\n    -H "Authorization: Bearer $HERCULES_API_KEY"',
      },
    },
  },
  {
    ...
    perLanguage: {
      typescript: {
        method: 'client.content.assets.create',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst asset = await client.content.assets.create({\n  filename: 'filename',\n  mime_type: 'mime_type',\n  size: -9007199254740991,\n});\n\nconsole.log(asset.id);",
      },
      http: {
  ...
  },
  {
    ...
    perLanguage: {
      typescript: {
        method: 'client.content.assets.update',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst asset = await client.content.assets.update('asset_id');\n\nconsole.log(asset.id);",
      },
      http: {
  ...
  },
  {
    ...
    perLanguage: {
      typescript: {
        method: 'client.content.assets.delete',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nawait client.content.assets.delete('asset_id');",
      },
      http: {
  ...
  },
  {
    ...
    perLanguage: {
      typescript: {
        method: 'client.content.assets.publish',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst asset = await client.content.assets.publish('asset_id');\n\nconsole.log(asset.id);",
      },
      http: {
        example:
          'curl https://api.hercules.app/v1/content/assets/$ASSET_ID/publish \\\n    -X POST \\\n    -H "Authorization: Bearer $HERCULES_API_KEY"',
      },
    },
  },
  {
    ...
    perLanguage: {
      typescript: {
        method: 'client.content.locales.list',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\n// Automatically fetches more pages as needed.\nfor await (const locale of client.content.locales.list()) {\n  console.log(locale.id);\n}",
      },
      http: {
        example:
          'curl https://api.hercules.app/v1/content/locales \\\n    -H "Authorization: Bearer $HERCULES_API_KEY"',
      },
    },
  },
  {
    ...
    perLanguage: {
      typescript: {
        method: 'client.content.locales.get',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst locale = await client.content.locales.get('locale_code');\n\nconsole.log(locale.id);",
      },
      http: {
        example:
          'curl https://api.hercules.app/v1/content/locales/$LOCALE_CODE \\\n    -H "Authorization: Bearer $HERCULES_API_KEY"',
      },
    },
  },
  {
    ...
    perLanguage: {
      typescript: {
        method: 'client.content.locales.create',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst locale = await client.content.locales.create({ code: 'xx', name: 'name' });\n\nconsole.log(locale.id);",
      },
      http: {
  ...
  },
  {
    ...
    perLanguage: {
      typescript: {
        method: 'client.content.locales.update',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst locale = await client.content.locales.update('locale_code');\n\nconsole.log(locale.id);",
      },
      http: {
  ...
  },
  {
    ...
    perLanguage: {
      typescript: {
        method: 'client.content.locales.delete',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nawait client.content.locales.delete('locale_code');",
      },
      http: {
  ...
  },
  {
    ...
    perLanguage: {
      typescript: {
        method: 'client.content.releases.list',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\n// Automatically fetches more pages as needed.\nfor await (const release of client.content.releases.list()) {\n  console.log(release.id);\n}",
      },
      http: {
        example:
          'curl https://api.hercules.app/v1/content/releases \\\n    -H "Authorization: Bearer $HERCULES_API_KEY"',
      },
    },
  },
  {
    ...
    perLanguage: {
      typescript: {
        method: 'client.content.releases.get',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst release = await client.content.releases.get('release_id');\n\nconsole.log(release.id);",
      },
      http: {
        example:
          'curl https://api.hercules.app/v1/content/releases/$RELEASE_ID \\\n    -H "Authorization: Bearer $HERCULES_API_KEY"',
      },
    },
  },
  {
    ...
    perLanguage: {
      typescript: {
        method: 'client.content.releases.create',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst release = await client.content.releases.create({ name: 'name' });\n\nconsole.log(release.id);",
      },
      http: {
  ...
  },
  {
    ...
    perLanguage: {
      typescript: {
        method: 'client.content.releases.update',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst release = await client.content.releases.update('release_id');\n\nconsole.log(release.id);",
      },
      http: {
  ...
  },
  {
    ...
    perLanguage: {
      typescript: {
        method: 'client.content.releases.delete',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nawait client.content.releases.delete('release_id');",
      },
      http: {
  ...
  },
  {
    ...
    perLanguage: {
      typescript: {
        method: 'client.content.releases.publish',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst release = await client.content.releases.publish('release_id');\n\nconsole.log(release.id);",
      },
      http: {
        example:
          'curl https://api.hercules.app/v1/content/releases/$RELEASE_ID/publish \\\n    -X POST \\\n    -H "Authorization: Bearer $HERCULES_API_KEY"',
      },
    },
  },
  {
    ...
    perLanguage: {
      typescript: {
        method: 'client.content.releases.schedule',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst release = await client.content.releases.schedule('release_id', {\n  scheduled_at: '2019-12-27T18:11:19.117Z',\n});\n\nconsole.log(release.id);",
      },
      http: {
        example:
          'curl https://api.hercules.app/v1/content/releases/$RELEASE_ID/schedule \\\n    -H \'Content-Type: application/json\' \\\n    -H "Authorization: Bearer $HERCULES_API_KEY" \\\n    -d \'{\n          "scheduled_at": "2019-12-27T18:11:19.117Z"\n        }\'',
      },
    },
  },
  {
    ...
    perLanguage: {
      typescript: {
        method: 'client.content.releases.items.add',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst releaseItem = await client.content.releases.items.add('release_id', {\n  item_id: 'item_id',\n  type: 'entry',\n});\n\nconsole.log(releaseItem.id);",
      },
      http: {
        example:
          'curl https://api.hercules.app/v1/content/releases/$RELEASE_ID/items \\\n    -H \'Content-Type: application/json\' \\\n    -H "Authorization: Bearer $HERCULES_API_KEY" \\\n    -d \'{\n          "item_id": "item_id",\n          "type": "entry"\n        }\'',
      },
    },
  },
  {
    ...
    perLanguage: {
      typescript: {
        method: 'client.content.releases.items.remove',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nawait client.content.releases.items.remove('item_id', { release_id: 'release_id' });",
      },
      http: {
        example:
          'curl https://api.hercules.app/v1/content/releases/$RELEASE_ID/items/$ITEM_ID \\\n    -X DELETE \\\n    -H "Authorization: Bearer $HERCULES_API_KEY"',
      },
    },
  },
  {
    ...
    perLanguage: {
      ...
      http: {
        example:
          'curl https://api.hercules.app/v1/domains \\\n    -H "Authorization: Bearer $HERCULES_API_KEY"',
      },
    },
  },
  {
  ...
  },
  {
    ...
    perLanguage: {
      ...
      http: {
        example:
          'curl https://api.hercules.app/v1/email \\\n    -H \'Content-Type: application/json\' \\\n    -H "Authorization: Bearer $HERCULES_API_KEY" \\\n    -d \'{\n          "from": "x",\n          "subject": "x",\n          "to": "dev@stainless.com"\n        }\'',
      },
    },
  },
  {
  ...
  },
  {
    ...
    response:
      "{ id: string; created_at: string; from: string; subject: string; to: string[]; bcc?: string[]; bounce_reason?: string; cc?: string[]; html?: string; last_event?: 'sent' | 'delivered' | 'opened' | 'clicked' | 'bounced' | 'complained'; reply_to?: string[]; text?: string; }",
    markdown:
      "## get\n\n`client.email.get(email_id: string): { id: string; created_at: string; from: string; subject: string; to: string[]; bcc?: string[]; bounce_reason?: string; cc?: string[]; html?: string; last_event?: 'sent' | 'delivered' | 'opened' | 'clicked' | 'bounced' | 'complained'; reply_to?: string[]; text?: string; }`\n\n**get** `/v1/email/{email_id}`\n\nRetrieves a single email by its unique identifier.\n\n### Parameters\n\n- `email_id: string`\n\n### Returns\n\n- `{ id: string; created_at: string; from: string; subject: string; to: string[]; bcc?: string[]; bounce_reason?: string; cc?: string[]; html?: string; last_event?: 'sent' | 'delivered' | 'opened' | 'clicked' | 'bounced' | 'complained'; reply_to?: string[]; text?: string; }`\n  A sent email object with full body content\n\n  - `id: string`\n  - `created_at: string`\n  - `from: string`\n  - `subject: string`\n  - `to: string[]`\n  - `bcc?: string[]`\n  - `bounce_reason?: string`\n  - `cc?: string[]`\n  - `html?: string`\n  - `last_event?: 'sent' | 'delivered' | 'opened' | 'clicked' | 'bounced' | 'complained'`\n  - `reply_to?: string[]`\n  - `text?: string`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst email = await client.email.get('email_id');\n\nconsole.log(email);\n```",
    perLanguage: {
      typescript: {
        method: 'client.email.get',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst email = await client.email.get('email_id');\n\nconsole.log(email.id);",
      },
      http: {
        example:
          'curl https://api.hercules.app/v1/email/$EMAIL_ID \\\n    -H "Authorization: Bearer $HERCULES_API_KEY"',
      },
    },
  },
  {
    ...
    response:
      "{ id: string; created_at: string; mail_from_enabled: boolean; receiving: { available: boolean; enabled: boolean; records: { name: string; status: 'pending' | 'verified' | 'failed'; type: 'TXT' | 'CNAME' | 'MX'; value: string; priority?: number; }[]; status: 'disabled' | 'pending' | 'verified' | 'failed'; }; status: 'pending' | 'verified' | 'failed'; type: 'email' | 'domain'; value: string; mail_from_subdomain?: string; verification_records?: { name: string; status: 'pending' | 'verified' | 'failed'; type: 'TXT' | 'CNAME' | 'MX'; value: string; priority?: number; }[]; warnings?: { code: 'dmarc_unaligned' | 'dmarc_public_domain'; domain: string; message: string; }[]; }",
    perLanguage: {
      typescript: {
        method: 'client.email.identities.create',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst identity = await client.email.identities.create({\n  type: 'email',\n  value: 'dev@stainless.com',\n});\n\nconsole.log(identity.id);",
      },
      http: {
        example:
          'curl https://api.hercules.app/v1/email/identities \\\n    -H \'Content-Type: application/json\' \\\n    -H "Authorization: Bearer $HERCULES_API_KEY" \\\n    -d \'{\n          "type": "email",\n          "value": "dev@stainless.com"\n        }\'',
      },
    },
  },
  {
    ...
    response:
      "{ id: string; created_at: string; mail_from_enabled: boolean; receiving: { available: boolean; enabled: boolean; records: { name: string; status: 'pending' | 'verified' | 'failed'; type: 'TXT' | 'CNAME' | 'MX'; value: string; priority?: number; }[]; status: 'disabled' | 'pending' | 'verified' | 'failed'; }; status: 'pending' | 'verified' | 'failed'; type: 'email' | 'domain'; value: string; mail_from_subdomain?: string; verification_records?: { name: string; status: 'pending' | 'verified' | 'failed'; type: 'TXT' | 'CNAME' | 'MX'; value: string; priority?: number; }[]; warnings?: { code: 'dmarc_unaligned' | 'dmarc_public_domain'; domain: string; message: string; }[]; }",
    markdown:
      "## list\n\n`client.email.identities.list(ending_before?: string, limit?: number, starting_after?: string): { id: string; created_at: string; mail_from_enabled: boolean; receiving: object; status: 'pending' | 'verified' | 'failed'; type: 'email' | 'domain'; value: string; mail_from_subdomain?: string; verification_records?: object[]; warnings?: object[]; }`\n\n**get** `/v1/email/identities`\n\nRetrieves a paginated list of sender identities (email addresses and domains) configured for this website.\n\n### Parameters\n\n- `ending_before?: string`\n  Cursor for backward pagination. Pass the ID of the first identity from the previous page.\n\n- `limit?: number`\n  Maximum number of identities to return (1-100)\n\n- `starting_after?: string`\n  Cursor for forward pagination. Pass the ID of the last identity from the previous page.\n\n### Returns\n\n- `{ id: string; created_at: string; mail_from_enabled: boolean; receiving: { available: boolean; enabled: boolean; records: { name: string; status: 'pending' | 'verified' | 'failed'; type: 'TXT' | 'CNAME' | 'MX'; value: string; priority?: number; }[]; status: 'disabled' | 'pending' | 'verified' | 'failed'; }; status: 'pending' | 'verified' | 'failed'; type: 'email' | 'domain'; value: string; mail_from_subdomain?: string; verification_records?: { name: string; status: 'pending' | 'verified' | 'failed'; type: 'TXT' | 'CNAME' | 'MX'; value: string; priority?: number; }[]; warnings?: { code: 'dmarc_unaligned' | 'dmarc_public_domain'; domain: string; message: string; }[]; }`\n  A verified sender identity (email address or domain) for sending emails\n\n  - `id: string`\n  - `created_at: string`\n  - `mail_from_enabled: boolean`\n  - `receiving: { available: boolean; enabled: boolean; records: { name: string; status: 'pending' | 'verified' | 'failed'; type: 'TXT' | 'CNAME' | 'MX'; value: string; priority?: number; }[]; status: 'disabled' | 'pending' | 'verified' | 'failed'; }`\n  - `status: 'pending' | 'verified' | 'failed'`\n  - `type: 'email' | 'domain'`\n  - `value: string`\n  - `mail_from_subdomain?: string`\n  - `verification_records?: { name: string; status: 'pending' | 'verified' | 'failed'; type: 'TXT' | 'CNAME' | 'MX'; value: string; priority?: number; }[]`\n  - `warnings?: { code: 'dmarc_unaligned' | 'dmarc_public_domain'; domain: string; message: string; }[]`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\n// Automatically fetches more pages as needed.\nfor await (const identity of client.email.identities.list()) {\n  console.log(identity);\n}\n```",
    perLanguage: {
      typescript: {
        method: 'client.email.identities.list',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\n// Automatically fetches more pages as needed.\nfor await (const identity of client.email.identities.list()) {\n  console.log(identity.id);\n}",
      },
      http: {
  ...
  },
  {
    ...
    response:
      "{ id: string; created_at: string; mail_from_enabled: boolean; receiving: { available: boolean; enabled: boolean; records: { name: string; status: 'pending' | 'verified' | 'failed'; type: 'TXT' | 'CNAME' | 'MX'; value: string; priority?: number; }[]; status: 'disabled' | 'pending' | 'verified' | 'failed'; }; status: 'pending' | 'verified' | 'failed'; type: 'email' | 'domain'; value: string; mail_from_subdomain?: string; verification_records?: { name: string; status: 'pending' | 'verified' | 'failed'; type: 'TXT' | 'CNAME' | 'MX'; value: string; priority?: number; }[]; warnings?: { code: 'dmarc_unaligned' | 'dmarc_public_domain'; domain: string; message: string; }[]; }",
    markdown:
      "## get\n\n`client.email.identities.get(identity_id: string): { id: string; created_at: string; mail_from_enabled: boolean; receiving: object; status: 'pending' | 'verified' | 'failed'; type: 'email' | 'domain'; value: string; mail_from_subdomain?: string; verification_records?: object[]; warnings?: object[]; }`\n\n**get** `/v1/email/identities/{identity_id}`\n\nRetrieves a sender identity by ID. Returns the identity with its current verification status and any required DNS records.\n\n### Parameters\n\n- `identity_id: string`\n\n### Returns\n\n- `{ id: string; created_at: string; mail_from_enabled: boolean; receiving: { available: boolean; enabled: boolean; records: { name: string; status: 'pending' | 'verified' | 'failed'; type: 'TXT' | 'CNAME' | 'MX'; value: string; priority?: number; }[]; status: 'disabled' | 'pending' | 'verified' | 'failed'; }; status: 'pending' | 'verified' | 'failed'; type: 'email' | 'domain'; value: string; mail_from_subdomain?: string; verification_records?: { name: string; status: 'pending' | 'verified' | 'failed'; type: 'TXT' | 'CNAME' | 'MX'; value: string; priority?: number; }[]; warnings?: { code: 'dmarc_unaligned' | 'dmarc_public_domain'; domain: string; message: string; }[]; }`\n  A verified sender identity (email address or domain) for sending emails\n\n  - `id: string`\n  - `created_at: string`\n  - `mail_from_enabled: boolean`\n  - `receiving: { available: boolean; enabled: boolean; records: { name: string; status: 'pending' | 'verified' | 'failed'; type: 'TXT' | 'CNAME' | 'MX'; value: string; priority?: number; }[]; status: 'disabled' | 'pending' | 'verified' | 'failed'; }`\n  - `status: 'pending' | 'verified' | 'failed'`\n  - `type: 'email' | 'domain'`\n  - `value: string`\n  - `mail_from_subdomain?: string`\n  - `verification_records?: { name: string; status: 'pending' | 'verified' | 'failed'; type: 'TXT' | 'CNAME' | 'MX'; value: string; priority?: number; }[]`\n  - `warnings?: { code: 'dmarc_unaligned' | 'dmarc_public_domain'; domain: string; message: string; }[]`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst identity = await client.email.identities.get('identity_id');\n\nconsole.log(identity);\n```",
    perLanguage: {
      typescript: {
        method: 'client.email.identities.get',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst identity = await client.email.identities.get('identity_id');\n\nconsole.log(identity.id);",
      },
      http: {
        example:
          'curl https://api.hercules.app/v1/email/identities/$IDENTITY_ID \\\n    -H "Authorization: Bearer $HERCULES_API_KEY"',
      },
    },
  },
  {
    ...
    response:
      "{ id: string; created_at: string; mail_from_enabled: boolean; receiving: { available: boolean; enabled: boolean; records: { name: string; status: 'pending' | 'verified' | 'failed'; type: 'TXT' | 'CNAME' | 'MX'; value: string; priority?: number; }[]; status: 'disabled' | 'pending' | 'verified' | 'failed'; }; status: 'pending' | 'verified' | 'failed'; type: 'email' | 'domain'; value: string; mail_from_subdomain?: string; verification_records?: { name: string; status: 'pending' | 'verified' | 'failed'; type: 'TXT' | 'CNAME' | 'MX'; value: string; priority?: number; }[]; warnings?: { code: 'dmarc_unaligned' | 'dmarc_public_domain'; domain: string; message: string; }[]; }",
    markdown:
      "## verify\n\n`client.email.identities.verify(identity_id: string, resend?: boolean): { id: string; created_at: string; mail_from_enabled: boolean; receiving: object; status: 'pending' | 'verified' | 'failed'; type: 'email' | 'domain'; value: string; mail_from_subdomain?: string; verification_records?: object[]; warnings?: object[]; }`\n\n**post** `/v1/email/identities/{identity_id}/verify`\n\nTriggers a manual recheck of the identity's verification status against AWS SES. Returns the identity with its updated status.\n\n### Parameters\n\n- `identity_id: string`\n\n- `resend?: boolean`\n  If true and the identity is an unverified email address, resend the verification email.\n\n### Returns\n\n- `{ id: string; created_at: string; mail_from_enabled: boolean; receiving: { available: boolean; enabled: boolean; records: { name: string; status: 'pending' | 'verified' | 'failed'; type: 'TXT' | 'CNAME' | 'MX'; value: string; priority?: number; }[]; status: 'disabled' | 'pending' | 'verified' | 'failed'; }; status: 'pending' | 'verified' | 'failed'; type: 'email' | 'domain'; value: string; mail_from_subdomain?: string; verification_records?: { name: string; status: 'pending' | 'verified' | 'failed'; type: 'TXT' | 'CNAME' | 'MX'; value: string; priority?: number; }[]; warnings?: { code: 'dmarc_unaligned' | 'dmarc_public_domain'; domain: string; message: string; }[]; }`\n  A verified sender identity (email address or domain) for sending emails\n\n  - `id: string`\n  - `created_at: string`\n  - `mail_from_enabled: boolean`\n  - `receiving: { available: boolean; enabled: boolean; records: { name: string; status: 'pending' | 'verified' | 'failed'; type: 'TXT' | 'CNAME' | 'MX'; value: string; priority?: number; }[]; status: 'disabled' | 'pending' | 'verified' | 'failed'; }`\n  - `status: 'pending' | 'verified' | 'failed'`\n  - `type: 'email' | 'domain'`\n  - `value: string`\n  - `mail_from_subdomain?: string`\n  - `verification_records?: { name: string; status: 'pending' | 'verified' | 'failed'; type: 'TXT' | 'CNAME' | 'MX'; value: string; priority?: number; }[]`\n  - `warnings?: { code: 'dmarc_unaligned' | 'dmarc_public_domain'; domain: string; message: string; }[]`\n\n### Example\n\n```typescript\nimport Hercules from '@usehercules/sdk';\n\nconst client = new Hercules();\n\nconst identity = await client.email.identities.verify('identity_id');\n\nconsole.log(identity);\n```",
    perLanguage: {
      typescript: {
        method: 'client.email.identities.verify',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst identity = await client.email.identities.verify('identity_id');\n\nconsole.log(identity.id);",
      },
      http: {
        example:
          "curl https://api.hercules.app/v1/email/identities/$IDENTITY_ID/verify \\\n    -H 'Content-Type: application/json' \\\n    -H \"Authorization: Bearer $HERCULES_API_KEY\" \\\n    -d '{}'",
      },
    },
  },
  {
    ...
    perLanguage: {
      typescript: {
        method: 'client.email.identities.delete',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nawait client.email.identities.delete('identity_id');",
      },
      http: {
  ...
  },
  {
    ...
    perLanguage: {
      ...
      http: {
        example:
          'curl https://api.hercules.app/v1/files \\\n    -H "Authorization: Bearer $HERCULES_API_KEY"',
      },
    },
  },
  {
    ...
    perLanguage: {
      ...
      http: {
        example:
          'curl https://api.hercules.app/v1/files/$FILE_ID \\\n    -H "Authorization: Bearer $HERCULES_API_KEY"',
      },
    },
  },
  {
    ...
    perLanguage: {
      typescript: {
        method: 'client.pushNotifications.enable',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst response = await client.pushNotifications.enable();\n\nconsole.log(response.vapidPublicKey);",
      },
      http: {
        example:
          'curl https://api.hercules.app/v1/push-notifications/enable \\\n    -X POST \\\n    -H "Authorization: Bearer $HERCULES_API_KEY"',
      },
    },
  },
  {
  ...
  },
  {
    ...
    perLanguage: {
      typescript: {
        method: 'client.pushNotifications.unsubscribe',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst response = await client.pushNotifications.unsubscribe({ secret: 'x' });\n\nconsole.log(response.success);",
      },
      http: {
        example:
          'curl https://api.hercules.app/v1/push-notifications/unsubscribe \\\n    -H \'Content-Type: application/json\' \\\n    -H "Authorization: Bearer $HERCULES_API_KEY" \\\n    -d \'{\n          "secret": "x"\n        }\'',
      },
    },
  },
  {
  ...
  },
  {
    ...
    perLanguage: {
      typescript: {
        method: 'client.pushNotifications.topics.subscribe',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst response = await client.pushNotifications.topics.subscribe({ topics: ['x'], visitorId: 'x' });\n\nconsole.log(response.topics);",
      },
      http: {
        example:
          'curl https://api.hercules.app/v1/push-notifications/topics/subscribe \\\n    -H \'Content-Type: application/json\' \\\n    -H "Authorization: Bearer $HERCULES_API_KEY" \\\n    -d \'{\n          "topics": [\n            "x"\n          ],\n          "visitorId": "x"\n        }\'',
      },
    },
  },
  {
    ...
    perLanguage: {
      typescript: {
        method: 'client.pushNotifications.topics.unsubscribe',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\nconst response = await client.pushNotifications.topics.unsubscribe({\n  topics: ['x'],\n  visitorId: 'x',\n});\n\nconsole.log(response.topics);",
      },
      http: {
        example:
          'curl https://api.hercules.app/v1/push-notifications/topics/unsubscribe \\\n    -H \'Content-Type: application/json\' \\\n    -H "Authorization: Bearer $HERCULES_API_KEY" \\\n    -d \'{\n          "topics": [\n            "x"\n          ],\n          "visitorId": "x"\n        }\'',
      },
    },
  },
  {
    ...
    perLanguage: {
      typescript: {
        method: 'client.pushNotifications.topics.list',
        example:
          "import Hercules from '@usehercules/sdk';\n\nconst client = new Hercules({\n  apiVersion: '2025-12-09',\n  apiKey: process.env['HERCULES_API_KEY'], // This is the default and can be omitted\n});\n\n// Automatically fetches more pages as needed.\nfor await (const topicListResponse of client.pushNotifications.topics.list({ visitorId: 'x' })) {\n  console.log(topicListResponse.id);\n}",
      },
      http: {
        example:
          'curl https://api.hercules.app/v1/push-notifications/topics \\\n    -H "Authorization: Bearer $HERCULES_API_KEY"',
      },
    },
  },
];
