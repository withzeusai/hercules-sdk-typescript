...
async function run(client) {
  const customer = await client.commerce.customers.create({ email: 'john.doe@example.com', name: 'John Doe' });
...
const remoteStainlessHandler = async (
  ...
  const localClientEnvs = { HERCULES_API_KEY: readEnv('HERCULES_API_KEY') ?? client.apiKey ?? undefined, HERCULES_BASE_URL: readEnv('HERCULES_BASE_URL') ?? client.baseURL ?? undefined };
...
const localDenoHandler = async (
  ...
  try {
    const sdkPkgName = '@usehercules/sdk';
