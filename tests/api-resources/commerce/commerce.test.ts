...
describe('resource commerce', () => {
  ...
  test.skip('cancel: only required params', async () => {
    const responsePromise = client.commerce.cancel({ customer_id: 'cus_1234567890', subscription_id: 'sub_1234567890' });
  ...
  test.skip('check: only required params', async () => {
    const responsePromise = client.commerce.check({ customer_id: 'cus_1234567890', resource_id: 'feat_1234567890' });
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
    const response = await responsePromise;
    expect(response).not.toBeInstanceOf(Response);
